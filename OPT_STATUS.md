...
- **Current tip #244** `9d2fcb5` carries the flat profile-v19 counts plus CPU-only structured-skip, prefetch-gate, AMP skip-reason, blocked/online gradient, sparse-probe, AUTO-environment, eager raw-score decode, explicit-backend AUTO-gate, top-k-one sampler-layout, distinct-Q/K score-V parity, and blocked-RoPE tile diagnostics; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
