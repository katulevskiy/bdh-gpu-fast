...
- **Current tip** `9b6d0a8` carries the flat profile-v20 counts plus the CPU-only force-CPU harness, prefetch gate, AMP state, generate-environment, batched blocked-gradient, sparse-boundary, AUTO-threshold fallback, score-V gradient, and signed raw-score decode contracts; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
