# OPT status — landed work (#1–#257; #160 docs scope retained)

Private sandbox only: [`katulevskiy/bdh-gpu-opt`](https://github.com/katulevskiy/bdh-gpu-opt).
**Do not** open PRs against `pathwaycom/bdh` or any `pathwaycom/*` repo.

Tip pointer: `b00dbd1` (#256 batched blocked-tile gradient parity, current tip) follows #257 sparse density-only crossover boundary (`042a01d`), #255 absent generate-impl environment cleanup (`1b67d13`), #254 active AMP state on CPU failure (`94a4e55`), #253 docs refresh through profile-v20/#251/#252 (`53491cf`), #252 CPU prefetch skip guard (`0baa35d`), #251 force-CPU GPU harness path (`84bd3f6`), profile-v20 (`f4c7cab`), #249 CUDA-mirror packed decode gradients (`567c1bf`), #248 docs refresh (`0c7c2f1`), and #247 shared-V strict-tril backward (`77a1ca4`). Profile-v20 remains the matched CPU-only re-profile: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`; all later landings are CPU-only contract or docs coverage. #251 keeps forced smoke on CPU even when CUDA is available; #252 short-circuits the CPU H2D gate before probing CUDA; #254 preserves an active AMP configuration on an unavailable CPU dtype switch; #255 restores an initially absent generate-impl environment variable after interruption; #256 extends blocked/online long-path gradient parity to B=2; and #257 covers the sparse density-only crossover boundary. None adds CUDA timing or GPU speedup evidence. Real GPU measurement remains the P0 blocker and cold CUDA/Triton validation remains open.
...
- **Current tip** `b00dbd1` carries the flat profile-v20 counts plus the CPU-only force-CPU harness, prefetch gate, AMP state, generate-environment, batched blocked-gradient, and sparse-boundary contracts; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
