# OPT status — landed work (#1–#674; docs-v93)
...
Attention remains **raw scores × `tril(diagonal=-1)`**: no softmax, no `1/√d`, and no SDPA (`F.scaled_dot_product_attention`).
...
|---|---|---|
| Device | CPU-only; CUDA/Triton paths skip or fall back cleanly here; structured GPU skips classify build/device/runtime state, retain the requested configuration, keep schema-v10 stdout/JSON skip records aligned with `timing_scope=none`, preserve failing CUDA build-probe, device, and availability-probe details, and keep `BDH_BUILD_CUDA` exact (`1` only); only `BDH_FORCE_CPU_EXT=1` selects CPU-only extension setup without probing CUDA availability, even when CUDA is detected, and that mode does not require `nvcc`; the exact force-CPU C++ sources, include path, and C++20 flags are covered with no CUDA metadata; CPU CUDA benchmark probes label the effective pure-PyTorch reference without a native extension | No GPU claim |
| Attention | `eager` default; `blocked`, `online`, `triton`, and `cuda` opt-in; strict-past backward now covers padded/strided/zero-stride Q/K/V, capacity-padded V sequence storage, Q/K backing-storage gradients, shared-Q batch/head-broadcast and zero-stride Q/K/V gradient reduction, batch/head-broadcast and zero-upstream plus sequence-broadcast upstream gradients, aliased Q/K long-prefill views, strided aliased-Q/K duplicate-path gradients with padded slots left zero, dispatcher-selected blocked/online partial long-prefill tiles, both sides of the first and second default 64-row tiles, strided upstream gradients, packed-decode V, empty-sequence contracts, multi-query upstream rows spanning tile boundaries, tiled shared/per-head online decode autograd, composed feature-strided Q/K/V storage gradients, sequence-strided Q/K/V views with base-storage scatter, and head-strided blocked/online prefill with skipped-head gradients across public aliases on CPU, including AUTO-selected blocked cold prefill over padded Q/K/V feature views with raw-score forward parity, analytic backward parity, preserved layout, and zero padding gradients, including combined zero-stride self-attention Q/K and V views whose duplicate/broadcast gradients reduce to shared bases, sequence-strided aliased Q/K duplicate paths with skipped slots left zero, zero-stride head-expanded shared-V decode, distinct feature-strided Q/K/V storage gradients, and batch-and-head broadcast upstream gradients | Raw strict-tril parity on CPU; GPU measure open |
| AUTO | Off by default; decode threshold remains independent from the optional cold threshold; blank/unset and zero-threshold transitions, strict cold/decode boundaries, including empty cold fallback and strict valid cold-only overrides, Triton preference when available, per-call requested-backend precedence including the `online` alias over a conflicting eager environment, runtime disable/re-enable transitions, threshold preservation, caller-argument isolation, nested probe restoration of outer cold/decode gates and the original environment, including restoration when an inner probe raises, omitted-cold-threshold cleanup after exceptions, and malformed/continuing threshold sweeps fail closed before execution; invalid cold thresholds do not poison the decode gate or override explicit blocked, online, Triton, or CUDA backends on cold or decode paths | CPU dispatch/parity only |
| RoPE | `eager` default; fused path opt-in; paired output-slot aliasing, overlapping direct/view/shifted output aliases, public T>1 cosine/sine aliases including cached-cis overlap rejection before stores, incompatible cis leading dimensions, explicit `impl` precedence over `BDH_ROPE_IMPL` for both public entrypoints, unsupported backend rejection before malformed-input handling, and CPU-safe blocked/Triton-fallback backward with pair-specific upstream gradients, non-contiguous pair-strided inputs, public T=1 backward on strided inputs, strided T>1 input gradients through both public dispatch implementations, and output cache slots are covered; mixed-dtype, strided-output parity is covered through the public T>1 dispatcher | CPU shape/parity only; fused GPU validation open |
| Compile | Off by default; invalid and normalized `BDH_COMPILE_PROBE` values fail closed to `train_bwd`, including missing-`example_y` training callers and missing-`example_x` no-probe callers; input-only forward probes succeed without targets, while eval probes forward optional target batches under no-grad; successful probes clear probe and pre-existing gradients and restore caller training state, eval probes across caller modes and target presence restore caller mode while preserving pre-existing parameter gradients, while failed backward and eval probes discard the wrapper, clear probe gradients, and preserve the eager module and caller mode; construction fallback preserves the eager module, caller mode, gradients, and compile options across both fullgraph settings; successful train_bwd probes retain wrapper/caller state across both fullgraph settings | CUDA graphs/inductor unmeasured |
| AMP | fp32/off by default; explicit `BDH_AMP_FORWARD_ONLY` off/on spellings remain CPU-safe; switching from active bf16/fp16 forward-only mode back to fp32 clears that mode, logits-only forward runs inside the forward-only context with cross-entropy afterward in fp32, the default full-forward path passes targets inside the AMP context, forward-only enabled-scaler hooks remain ordered while CE stays fp32 and parameters update before gradients clear, and a live CUDA runtime is required before exposing GPU throughput while failed context construction preserves the complete prior AMP state, CPU contexts remain contract-covered, and a disabled GradScaler uses the unscaled optimizer path while still clearing gradients, including scaler-update failures, unscaled optimizer-step failures, and failed-forward stale-gradient cleanup | CPU smoke only; GPU train throughput open |
| Sparse ReLU | Off by default; empty and explicit falsey gate values keep the optional probe disabled; normalized whitespace- and case-insensitive truthy values are the explicit opt-in; the disabled gate wins over guardrail enforcement without starting training or crossover work; explicit probe guardrails distinguish no-sample and terminal density-floor failure exits, including CLI floor overrides, while a missed density guardrail stays diagnostic-only by default, a passing sample forwards CLI sampling options, and only the optional mocked CPU crossover sweep is permitted | CPU density/control flow only; no sparse-kernel result |
| DataLoader | Validation split forwarding and repeated worker-backed batches preserve CPU tensor identity; terminal asynchronous producer failures reach every pending `next()` promptly with the host-gather cause; worker initialization locks full batches, bounded one-slot prefetch, persistent workers, and distinct reproducible CPU RNG streams; zero-worker loading keeps `batch_size=None` so each item remains a full host batch and its iterator cleanup is explicit and idempotent; worker construction preserves the full-batch iterable and requested split, shutdown joins the producer before clearing its handle, stops production, and clears the one-slot queue, including after a worker-prefetched batch; terminal producer failure leaves no live thread or queued resources, synchronous shutdown clears preloaded host/device slots, explicit DataLoader close stops workers and is idempotent, while negative `NUM_WORKERS` stays synchronous with pinning and persistent prefetch disabled | CPU identity/opt-out contracts only; H2D overlap unmeasured |
| Sampling | Strided logits and sampler probability buffers across multinomial, narrow/full/overflow-clamped top-k including `top_k=1`, zero-stride, and combined padded non-unit-vocabulary scratch with strided singleton output views preserve RNG parity, identity, contiguous-reference parity, and neighbor isolation, including narrow top-k reuse, padded scratch protection, the deeper padded narrow top-k case, combined padded non-unit-vocabulary logits/probability buffers with strided output, the `top_k=1` zero-stride decode-output case, offset non-unit-vocabulary logits views, and transposed padded probability scratch including the top-k-one case | CPU contract only |
| Packed generate | Cache path remains cat-free (`aten::cat=0`); interrupted cat probes restore their hook; decode-only AUTO overrides remain isolated; packed decode preserves raw score×V semantics across capacity-strided V layouts; threshold sweeps continue after individual failures while retaining aggregate failure; omitted decode/cold thresholds are not invented or leaked, including after an interrupted override; no-sweep AUTO and native-Triton probe paths preserve their dispatch contracts; `--device auto` falls back to CPU, explicit `--device cpu` remains CPU even when CUDA is visible, unavailable explicit CUDA requests skip cleanly, and CPU CUDA benchmark requests identify the effective pure-PyTorch fallback without a GPU claim | CPU operator contract, not GPU timing |

The retained profile-v20 baseline is `aten::copy_` 2/call for attention, 12/call for forward, and 394/call for generate, with `aten::cat=0` and `aten::contiguous=0`. The #403–#583, #589–#605, #607–#616, #622–#632, #633–#648, and #649–#674 additions are CPU-safe contracts, diagnostics, and docs; #450/#465/#479/#495/#512/#525/#540/#603/#616/#632/#647/#660/#674 refine CUDA-build/device/runtime skip handoffs, exact build-flag handling, schema-v10 requested-config retention, stdout/JSON parity, preserved device, availability-probe, build-probe, and no-visible-device errors, and no-timing boundaries, while #552/#566/#582/#598/#611/#626/#642/#656/#669 harden explicit CPU extension selection and #579/#670 label CPU-safe CUDA/Triton benchmark probes. #633/#648/#665 preserve explicit backend behavior around invalid or disabled AUTO thresholds; #651 is the prior docs refresh. None adds CUDA timing, GPU throughput, sparse-kernel evidence, or cold CUDA–Triton validation. These are CPU call-count observations, not GPU performance claims.
...
| PR | Branch / scope | What the matrix records | Result |
|---:|---|---|---|
| **#573** | `opt/blocked-tile-v27` / blocked prefill | Dispatcher-selected blocked/online prefill preserves raw strict-tril parity and Q/K/V gradients for capacity-padded V sequence storage | CPU contract only; no GPU claim |
| **#574** | `opt/online-decode-v28` / online decode | Tiled online decode preserves raw score×V parity for offset-strided query views without mutating the query | CPU raw score×V contract only; no GPU claim |
| **#575** | `opt/auto-thr-v28` / AUTO | An invalid cold-threshold override does not disturb an explicit blocked, Triton, CUDA, or online backend choice | CPU dispatch contract only; no GPU claim |
| **#576** | `opt/scorev-v28` / score-V | Dispatch aliases preserve feature-strided V storage gradients for shared and per-head V layouts under raw score×strict-tril×V semantics | CPU contract only; no GPU claim |
| **#577** | `opt/rope-v27` / RoPE | Paired T=1 fused rotation preserves eager parity for strided pair inputs and output cache slots without touching padding | CPU contract only; fused GPU validation open |
| **#578** | `opt/layout-v27` / sampling | Multinomial and top-k sampling preserve parity and storage neighbors for offset, non-unit-vocabulary logits views | CPU contract only |
| **#579** | `opt/gen-bench-v26` / generate benchmark | CPU CUDA probes remain runnable and label the effective pure-PyTorch reference when no native extension is present | CPU device-resolution contract only; no GPU claim |
| **#580** | `opt/compile-v28` / compile | A failed backward probe discards the compiled wrapper, clears probe gradients, and restores the caller training state | CPU contract only; GPU path unmeasured |
| **#581** | `opt/attn-bwd-v27` / attention backward | Zero upstream gradients produce zero Q/K/V gradients across implementation selectors | CPU contract only; no GPU claim |
| **#582** | `opt/cuda-build-v26` / CUDA build | Exact force-CPU setup does not require `nvcc` when CUDA is detected but no toolkit is available | CPU setup contract only; no GPU claim |
| **#583** | `opt/prefetch-v27` / DataLoader | Synchronous prefetch shutdown clears the preloaded host/device slots and remains idempotent | CPU contract only; H2D overlap unmeasured |

## Refresh additions (#589–#605)

| PR | Branch / scope | What the matrix records | Result |
|---:|---|---|---|
| **#589** | `opt/auto-thr-v29` / AUTO | Empty cold-threshold overrides follow the live shared threshold as it changes | CPU dispatch contract only |
| **#590** | `opt/online-decode-v29` / online decode | Tiled shared-V decode preserves raw score×V output and autograd parity | CPU raw score×V contract only; no GPU claim |
| **#591** | `opt/scorev-v29` / score-V | Feature-strided Q/K backing-storage gradients remain eager-parity across public dispatch aliases | CPU contract only; no GPU claim |
| **#592** | `opt/layout-v28` / sampling | Padded non-unit-vocabulary logits and probability buffers remain isolated with strided decode output | CPU contract only |
| **#593** | `opt/gen-bench-v27` / generate benchmark | Interrupted AUTO overrides restore an omitted cold-threshold environment entry cleanly | CPU control-flow contract only |
| **#594** | `opt/rope-v28` / RoPE | Public paired T=1 backward preserves eager parity for arbitrary upstream gradients through strided inputs | CPU contract only; fused GPU validation open |
| **#595** | `opt/compile-v29` / compile | Eval probes run under no-grad and restore the caller training mode | CPU contract only; GPU path unmeasured |
| **#596** | `opt/attn-bwd-v28` / attention backward | Sequence-broadcast upstream gradients preserve strict-tril backward parity | CPU contract only; no GPU claim |
| **#597** | `opt/prefetch-v28` / DataLoader | Explicit DataLoader shutdown stops persistent workers and remains idempotent | CPU contract only; H2D overlap unmeasured |
| **#598** | `opt/cuda-build-v27` / CUDA build | Exact force-CPU mode avoids CUDA availability probing and selects CPU-only setup | CPU setup contract only; no GPU claim |
| **#599** | `opt/amp-train-v29` / AMP | A disabled GradScaler uses the unscaled optimizer path and clears gradients | CPU contract only; no GPU throughput |
| **#600** | `opt/sparse-probe-v29` / sparse probe | Density-only guardrails forward CLI sampling options without starting crossover work | CPU guardrail only; no GPU claim |
| **#601** | `opt/blocked-tile-v29` / blocked prefill | Aliased Q/K long prefill preserves blocked/online raw strict-tril backward parity | CPU contract only; no GPU claim |
| **#602** | `opt/auto-thr-v30` / AUTO | A valid cold-only threshold has strict boundaries without moving the shared decode threshold | CPU dispatch contract only; no GPU claim |
| **#603** | `opt/gpu-measure-v27` / GPU measurement | Schema-v10 skip handoffs preserve CUDA device-probe errors in stdout and JSON | CPU diagnostics only; no GPU timing |
| **#604** | `opt/online-decode-v30` / online decode | Tiled per-head-V decode preserves raw score×V output and autograd parity | CPU raw score×V contract only; no GPU claim |
| **#605** | `opt/scorev-v30` / score-V | Composed feature-strided Q/K/V views preserve output and backing-storage gradients across dispatch aliases | CPU contract only; no GPU claim |

## Refresh additions (#607–#616)

| PR | Branch / scope | What the matrix records | Result |
|---:|---|---|---|
| **#607** | `opt/layout-v29` / sampling | Composed padded, non-unit-vocabulary logits/probability buffers remain isolated through the narrow `top_k=1` path with a strided decode output | CPU contract only |
| **#608** | `opt/rope-fuse-v29` / RoPE | Public RoPE dispatch rejects an unsupported explicit backend before touching malformed inputs | CPU dispatch contract only; fused GPU validation open |
| **#609** | `opt/gen-bench-v28` / AUTO | Nested AUTO probes restore outer cold/decode gates and the original environment | CPU control-flow contract only |
| **#610** | `opt/compile-v30` / compile | Eval probes forward optional targets under no-grad while restoring caller state | CPU contract only; GPU path unmeasured |
| **#611** | `opt/cuda-build-v28` / CUDA build | Force-CPU setup registers only C++ sources and excludes CUDA setup even when CUDA flags are present | CPU setup contract only; no GPU claim |
| **#612** | `opt/attn-bwd-v29` / attention backward | Aliased strided Q/K paths route duplicate gradients into backing storage while padded slots remain zero | CPU contract only; no GPU claim |
| **#613** | `opt/prefetch-v29` / DataLoader | Zero-worker DataLoader iterator shutdown is explicit and idempotent | CPU contract only; H2D overlap unmeasured |
| **#614** | `opt/amp-train-v30` / AMP | Forward-only enabled-scaler ordering keeps CE in fp32, updates parameters, and clears gradients | CPU contract only; no GPU throughput |
| **#615** | `opt/sparse-probe-v30` / sparse probe | A missed density guardrail remains diagnostic-only by default while optional CPU crossover work can still run | CPU guardrail only; no GPU claim |
| **#616** | `opt/gpu-measure-v28` / GPU measurement | CUDA availability-probe failures remain structured CPU-safe skip diagnostics without substituting CPU timing | CPU diagnostics only; no GPU timing |

## Refresh additions (#622–#632; docs-v91)

| PR | Branch / scope | What the matrix records | Result |
|---:|---|---|---|
| **#622** | `opt/layout-v30` / sampling | Padded non-unit-vocabulary sampler storage covers `top_k=1` with a zero-stride decode output | CPU contract only |
| **#623** | `opt/rope-fuse-v30` / RoPE | Explicit `impl` takes precedence over `BDH_ROPE_IMPL` through both public RoPE dispatch entrypoints | CPU dispatch contract only; fused GPU validation open |
| **#624** | `opt/gen-bench-v29` / AUTO | Nested AUTO overrides restore outer gates and the original environment when an inner probe raises | CPU control-flow contract only; no GPU claim |
| **#625** | `opt/compile-v31` / compile | Eval probes cover caller modes and target presence while preserving pre-existing parameter gradients under no-grad | CPU contract only; GPU path unmeasured |
| **#626** | `opt/cuda-build-v29` / CUDA build | Exact force-CPU mode emits only C++ compile metadata, excluding CUDA flags and macros even when CUDA and `nvcc` are detected | CPU setup contract only; no GPU claim |
| **#627** | `opt/prefetch-v30` / DataLoader | Async shutdown joins the producer before clearing its handle | CPU contract only; H2D overlap unmeasured |
| **#628** | `opt/attn-bwd-v30` / attention backward | Combined zero-stride self-attention Q/K and V views reduce duplicate and broadcast gradients to shared bases across implementations | CPU contract only; no GPU claim |
| **#629** | `opt/amp-train-v31` / AMP | AMP train-step cleanup clears optimizer gradients when processing or the unscaled optimizer step raises | CPU contract only; no GPU throughput |
| **#630** | `opt/sparse-probe-v31` / sparse probe | Empty and falsey gates keep the optional probe off; enforced mode does no training or crossover work while disabled | CPU guardrail only; no GPU claim |
| **#631** | `opt/blocked-tile-v31` / blocked prefill | AUTO-selected blocked cold prefill covers padded Q/K/V feature views with raw-score forward parity, analytic backward parity, layout preservation, and zero padding gradients | CPU contract only; no GPU claim |
| **#632** | `opt/gpu-measure-v29` / GPU measurement | CUDA build-probe exceptions remain in structured skip diagnostics with explicit no-timing scope | CPU diagnostics only; no GPU timing |
...
| PR | Branch / scope | What the matrix records | Result |
|---:|---|---|---|
| **#633** | `opt/auto-thr-v32` / AUTO | Explicit resolver backends remain authoritative when AUTO threshold configuration is invalid on cold and decode paths | CPU dispatch contract only; no GPU claim |
| **#634** | `opt/online-decode-v32` / online decode | Offset-packed per-head-V cache views preserve raw score×V output and Q/K/V gradients against the eager reference | CPU contract only; no GPU claim |
| **#636** | `opt/scorev-v32` / score-V | Capacity-strided self-alias Q storage preserves raw strict-tril score×V output and shared/per-head V storage gradients | CPU contract only; no GPU claim |
| **#637** | `opt/layout-v31` / sampling | Padded non-unit-vocabulary logits cover the missing `top_k=1` case while preserving neighbor isolation and sampling equivalence | CPU contract only |
| **#638** | `opt/gen-bench-v30` / AUTO | Nested cold-only overrides restore outer gates and the original environment after an interrupted inner probe | CPU control-flow contract only; no GPU claim |
| **#639** | `opt/rope-fuse-v31` / RoPE | Strided T>1 input gradients preserve eager parity through both public RoPE dispatch implementations | CPU contract only; fused GPU validation open |
| **#640** | `opt/compile-v32` / compile | Failed eval probes restore caller mode and preserve existing gradients while falling back to eager execution | CPU contract only; GPU path unmeasured |
| **#641** | `opt/attn-bwd-v31` / attention backward | Sequence-strided Q/K/V views preserve forward/backward parity and scatter gradients into base storage without touching skipped slots | CPU contract only; no GPU claim |
| **#642** | `opt/cuda-build-v30` / CUDA build | Force-CPU setup requires exact C++ sources, include path, and C++20 flags, with no CUDA metadata | CPU setup contract only; no GPU claim |
| **#643** | `opt/prefetch-v31` / DataLoader | Shutdown remains clean after workers have prefetched a batch, stopping workers and clearing the iterator | CPU contract only; H2D overlap unmeasured |
| **#644** | `opt/amp-train-v32` / AMP | Scaler-update failure preserves ordering and still clears gradients | CPU contract only; no GPU throughput |
| **#645** | `opt/sparse-probe-v32` / sparse probe | Whitespace- and case-normalized truthy values are the explicit opt-in while density-only mode remains isolated from crossover work | CPU guardrail only; no GPU claim |
| **#646** | `opt/blocked-tile-v32` / blocked prefill | Head-strided blocked/online prefill preserves strict-tril output/backward parity and zero gradients in skipped head storage | CPU contract only; no GPU claim |
| **#647** | `opt/gpu-measure-v30` / GPU measurement | A failed CUDA device probe skips before measurement while preserving structured runtime diagnostics and no-timing boundaries | CPU diagnostics only; no GPU timing |
| **#648** | `opt/auto-thr-v33` / AUTO | Explicit blocked, online, Triton, and CUDA backends bypass invalid AUTO threshold configuration on cold and decode paths | CPU dispatch contract only; no GPU claim |

## Refresh additions (#649–#674; docs-v93)

| PR / tip | Branch / scope | What the matrix records | Result |
|---|---|---|---|
| **#649** | `opt/scorev-v33` / score-V | Feature-strided self-alias Q/K score×V dispatch preserves raw strict-tril outputs and analytic gradients for shared and per-head V | CPU contract only; no GPU claim |
| **#650** | `opt/online-decode-v33` / online decode | Head-expanded zero-stride shared-V layouts preserve raw score×V parity across layouts and tile sizes | CPU contract only; no GPU claim |
| **#651** | `opt/docs-v92` / docs | Previous matrix/backlog refresh through #648 | Docs-only; P0 unchanged |
| **#652** | `opt/rope-fuse-v32` / RoPE | Fresh outputs remain dense for strided inputs through CPU-safe fused paths | CPU contract only; fused GPU validation open |
| **#653** | `opt/layout-v32` / sampling | Transposed, padded probability scratch remains isolated across full-vocab, overflow, narrow top-k, and strided decode-output cases | CPU contract only |
| **#654** | `opt/gen-bench-v32` / generate benchmark | The no-sweep AUTO path delegates unchanged arguments and propagates single-run status | CPU dispatch contract only; no GPU claim |
| **#655** | `opt/compile-v33` / compile | Construction failures preserve the eager module, caller mode, and pre-existing gradients across eval and training callers | CPU contract only; GPU path unmeasured |
| **#656** | `opt/cuda-build-v31` / CUDA build | Explicit CUDA setup registers CPU/CUDA sources, include paths, compiler flags, and `WITH_CUDA` metadata | CPU setup contract only; no GPU claim |
| **#657** | `opt/attn-bwd-v32` / attention backward | Aliased sequence-strided Q/K backward scatters duplicate paths to base storage while skipped slots remain zero | CPU contract only; no GPU claim |
| **#658** | `opt/prefetch-v32` / DataLoader | Async shutdown joins its producer after a consumed batch triggers refill | CPU contract only; H2D overlap unmeasured |
| **#659** | `opt/amp-train-v33` / AMP | Scaled optimizer-step failure retains CPU-safe cleanup and clears gradients | CPU contract only; no GPU throughput |
| **#660** | `opt/gpu-measure-v31` / GPU measurement | CUDA build-probe failure skips before device selection or measurement with structured diagnostics | CPU diagnostics only; no GPU timing |
| **#661** | `opt/sparse-probe-v33` / sparse probe | Sparse-probe contracts remain CPU-safe and make no GPU performance claim | CPU guardrail only; no GPU claim |
| **#662** | `opt/blocked-tile-v33` / blocked prefill | Explicit Triton cold fallback preserves raw-score strict-tril parity for shared and head-matched V layouts | CPU contract only; no GPU claim |
| **#663** | `opt/online-decode-v34` / online decode | Tiled decode with zero-stride head-expanded shared-V views preserves raw score×V output and gradients | CPU contract only; no GPU claim |
| **#664** | `opt/scorev-v34` / score-V | Distinct feature-strided Q/K/V storage preserves raw strict-tril score×V output and storage gradients across dispatch aliases | CPU contract only; no GPU claim |
| **#665** | `opt/auto-thr-v34` / AUTO | Disabled AUTO leaves eager dispatch unchanged even when dormant threshold variables are malformed on cold and decode paths | CPU dispatch contract only; no GPU claim |
| **#666** | `opt/layout-v33` / sampling | The missing `top_k=1` transposed scratch case preserves output layout, sentinels, and sampling equivalence | CPU contract only |
| **#667** | `opt/rope-fuse-v33` / RoPE | T=1 paired and unpaired CPU-safe paths keep fresh outputs dense and eager-parity for strided inputs | CPU contract only; fused GPU validation open |
| **#668** | `opt/compile-v34` / compile | Successful `train_bwd` probes preserve wrapper/caller state and gradient cleanup across both `fullgraph` settings | CPU contract only; GPU path unmeasured |
| **#669** | `opt/cuda-build-v32` / CUDA build | CPU-only native opt-in passes only CPU sources and compile metadata when CUDA is unavailable | CPU setup contract only; no GPU claim |
| **#670** | `opt/gen-bench-v32` / generate benchmark | Native Triton selection is covered by a CPU-safe CUDA device descriptor without GPU execution | CPU dispatch contract only; no GPU claim |
| **#671** | `opt/attn-bwd-v33` / attention backward | Aliased Q/K duplicate gradients reduce correctly with batch-and-head broadcast upstream gradients under raw strict-tril semantics | CPU contract only; no GPU claim |
| **#672** | `opt/prefetch-v33` / DataLoader | Async shutdown joins the producer after a refill fills the one-slot queue | CPU contract only; H2D overlap unmeasured |
| **#673** | `opt/amp-train-v34` / AMP | Failed model forward paths clear stale gradients while retaining CPU-safe AMP cleanup | CPU contract only; no GPU throughput |
| **#674 / tip `6683d335`** | `opt/gpu-measure-v32` / GPU measurement | `cuda_available=True` with zero visible devices is a structured diagnostics-only skip before device selection or timing | CPU diagnostics only; no GPU timing |
