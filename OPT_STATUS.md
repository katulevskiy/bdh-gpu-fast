# OPT status — landed work (#1–#249; #160 docs scope retained)

Private sandbox only: [`katulevskiy/bdh-gpu-opt`](https://github.com/katulevskiy/bdh-gpu-opt).
**Do not** open PRs against `pathwaycom/bdh` or any `pathwaycom/*` repo.

Tip pointer: `f4c7cab` (profile-v20, current tip) follows profile-v20 (`f4c7cab`), #249 CUDA-mirror packed decode gradients (`567c1bf`), #248 docs refresh (`0c7c2f1`), #247 shared-V strict-tril backward (`77a1ca4`), #245 successful compile-backward probe cleanup (`4047ce9`), #244 blocked RoPE tile contract (`9d2fcb5`), #242 distinct-Q/K score-V parity (`c20f440`), #241 top-k-one sampler layout coverage (`8a2ea37`), #240 AUTO cold explicit-backend contract (`2a004c3`), #239 eager raw-score decode contract (`34a0d9f`), #238 generate-AUTO environment cleanup (`95ecae0`), #237 sparse density-only probe contract (`66c1680`), #236 docs refresh through #235 (`02bcf68`), #235 long blocked/online gradient parity (`ee586d0`), #234 CPU AMP skip-reason contract (`1f1edd9`), #233 prefetch-v5 CUDA skip diagnostics (`ff5b071`), and #232 structured backend skip results (`efbb1f3`). Profile-v20 remains a matched CPU-only re-profile: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`; the result is flat versus profile-v19 and profile-v18. #237 adds a CPU-safe density-only sparse probe contract without changing opt-in or production sparse wiring; #238 adds CPU-only AUTO environment cleanup coverage; #239 extends the CPU raw-score decode contract to eager; #240 verifies explicit backends remain authoritative on both AUTO gates; #241 covers the top_k=1 sampler-layout boundary; #242 covers distinct, non-aliased Q/K score-V parity; #244 enforces positive blocked RoPE tiles before the T=1 shortcut; #245 verifies successful compile-backward probes restore caller mode and clear parameter grads; the current CUDA-build tip deepens the directory-named-nvcc no-op skip; profile-v20 records unchanged CPU operator counts after #249. None adds CUDA timing or GPU speedup evidence. Real GPU measurement remains the P0 blocker and cold CUDA/Triton validation remains open.
...
| **tip** | `OPT_NOTES.md` (profile-v20) | Record matched CPU operator counts after #249: attention/forward/generate `copy_`=2/12/394 per call, `cat=0`, `contiguous=0`; flat versus profile-v19 | CPU-only profile evidence; no GPU timing or speedup claim | **P0** real GPU measurement / cold CUDA-Triton validation remains open
...
- **Current tip** `f4c7cab` carries the flat profile-v20 counts plus CPU-only structured-skip, prefetch-gate, AMP skip-reason, blocked/online gradient, sparse-probe, AUTO-environment, eager raw-score decode, explicit-backend AUTO-gate, top-k-one sampler-layout, distinct-Q/K score-V parity, blocked-RoPE tile, successful compile-backward cleanup, CUDA missing-`nvcc` skip diagnostics, and CUDA-mirror packed decode gradient coverage; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
