# OPT status — landed work (#1–#342; #160 docs scope retained)

Private sandbox only: [`katulevskiy/bdh-gpu-opt`](https://github.com/katulevskiy/bdh-gpu-opt).
**Do not** open PRs against `pathwaycom/bdh` or any `pathwaycom/*` repo.

Tip pointer: `59030a3` (#342 forward-only AMP CE input contract) follows #341 CPU DataLoader worker H2D coverage, #340 GPU-bench timing skip coverage, #339 T=1 decode dtype coverage, #338 compile default-option coverage, #337 half-dtype attention backward coverage, #336 CUDA-build opt-in coverage, #335 gen-bench prompt validation, #334 paired RoPE gradient coverage, #333 strided sampler-layout coverage, and #332 docs-v70. The sandbox is CPU-only (`cuda=False`), so these are CPU-safe contracts only: real GPU measurement and cold CUDA–Triton validation remain the P0 blocker.
...
## Landed opts (#1–#342; prefetch, docs-v70, sparse, AMP, blocked, AUTO, packed shared-V, per-head score×V, layout, RoPE, compile, CUDA-build, duplicate-Q, forced-CPU measurement, packed decode dtype, score×V gradients, sampler layout, GPU-harness skips, CPU DataLoader worker coverage, and forward-only AMP CE coverage)
...
| **277** | `OPT_STATUS.md` / `OPT_BACKLOG.md` | Extend the optimization matrix and ranked backlog through #275 | Docs only; no GPU evidence | —
| **278** | `tests/test_auto_threshold_contract_v8.py` | Cover explicit AUTO cold-threshold independence and fallback to a later decode threshold when unset | CPU-safe contract coverage; no GPU timing or threshold/tile claim | **P0** GPU threshold/tile validation remains open
| **279** | `tests/test_fuse_scorev.py` | Verify blocked/online score×V output and Q/V gradients match eager when Q and K alias | CPU-only gradient parity; no GPU timing or performance claim | **P0** real GPU measurement / cold CUDA-Triton validation remains open
| **280** | `tests/test_online_decode_v5.py` | Cover multi-query signed raw-score decode against past-only per-head values across supported backends | CPU-only decode contract; no GPU timing or performance claim | **P0** real GPU measurement / cold CUDA-Triton validation remains open
| **281** | `tests/test_layout_v3.py` | Cover default and top-k sampler writes into a non-contiguous decode narrow while preserving reference output | CPU-only layout contract; no GPU timing or performance claim | **P0** real GPU measurement / cold CUDA-Triton validation remains open
| **282** | `tests/test_rope_fuse.py` | Cover fp16 RoPE writes into non-contiguous fp32 cache slots across CPU entrypoints and sequence shapes | CPU-only mixed-dtype parity contract; no GPU timing or fused-RoPE claim | **P0** GPU fused-RoPE validation remains open
| **283** | `tests/test_compile_v8_contract.py` | Cover train_bwd no-loss fallback with clean gradient and caller-mode restoration | CPU-only compile-probe contract; no GPU timing or CUDA-graph claim | **P1** GPU inductor / CUDA-graph validation remains open
| **284** | `tests/test_cuda_build.py` | Cover a non-executable PATH `nvcc` entry as a clear CPU-safe no-op before CUDA extension setup | CPU-only setup/skip contract; no GPU build or timing claim | **P0** GPU build/measurement remains open
| **285** | docs refresh | Refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #282 | Docs only | — |
| **286** | docs refresh | Record the optimization contracts through #284 | Docs only | — |
| **287** | `tests/test_attn_bwd_v4_contract.py` | CPU-only raw strict-tril score/backward contract across eager, blocked, online, Triton fallback, and CUDA-reference dispatch; no GPU timing or speedup claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **288** | `tests/test_inc_decode.py` | CPU-safe autograd parity for capacity-strided shared-V T=1 decode tiles with B>1; no GPU timing or performance claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **289** | `tests/test_amp_train_contracts_v9.py` | CPU-safe AMP forward-only environment contract; no GPU throughput claim and defaults unchanged | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **290** | `benchmarks/bench_gpu_attn.py` | Keep GPU-harness synchronization device-local and lock the CPU smoke contract; no GPU timing result on this box | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **291** | `tests/test_dataloader.py` | Global prefetch H2D opt-out skips CUDA probing and stream/event allocation for a configured CUDA device; CPU-safe coverage only | **P1** GPU H2D overlap/throughput remains unmeasured |
| **292** | `tests/test_sparse_probe_contract_v8.py` | CPU-only inclusive sparse-density guardrail threshold coverage; sparse remains opt-in and no GPU performance claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **293** | `tests/test_prefill_blocked.py` | CPU-only batched strict-tril raw-score coverage across broadcast/head-matched V at a non-divisible partial tile; no GPU performance claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **294** | `tests/test_bench_generate.py` | CPU-safe fail-closed validation for invalid AUTO prompt and threshold inputs before model/device setup; no GPU performance claim | **P1** GPU generate measurement remains open |
| **295** | `tests/test_auto_threshold_contract_v9.py` | Cover empty and whitespace-only `BDH_ATTN_AUTO_COLD_THRESHOLD` fallbacks to the live decode threshold, including strict-equality and above-threshold cold dispatch after a live decode-threshold update | CPU-only contract coverage; no GPU timing or threshold/tile claim | **P0** GPU threshold/tile validation remains open |
...
| **324** | `opt/docs-v69` | Refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #323 | Docs only; no GPU evidence | — |
| **325** | `tests/test_dataloader.py` | Deepen CPU DataLoader H2D identity coverage: no CUDA probing or CUDA object construction on the CPU path | CPU-only identity/no-allocation contract; no H2D timing or overlap claim | **P1** GPU H2D overlap remains unmeasured |
| **326** | `tests/test_amp_train_contracts_v11.py` | Cover the forward-only AMP train step: logits-only model invocation, fp32 cross-entropy outside autocast, and post-step gradient clearing | CPU-only AMP contract; no GPU throughput claim | **P3** GPU AMP train measurement remains open |
| **327** | `tests/test_prefill_blocked.py` | Extend blocked/prefill AUTO parity to shared and head-matched V layouts while preserving raw strict `tril(-1)` semantics | CPU-only blocked-dispatch parity; no GPU performance claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **328** | `tests/test_sparse_probe_contract_v10.py` | Keep false-like `BDH_SPARSE_PROBE` values (`empty`, `0`, `false`, `no`, `off`, `disabled`) on the default-off no-op path | CPU-only sparse gate coverage; no training/crossover or GPU claim | **P0** real GPU measurement / sparse validation remains open |
| **329** | `tests/test_auto_threshold_contract_v11.py` | Cover malformed shared AUTO threshold overrides across decode and cold dispatch, then recover after a valid update at the strict boundary | CPU-only AUTO threshold contract; no GPU timing or threshold claim | **P0** GPU threshold/tile validation remains open |
| **330** | `tests/test_online_decode_v5.py` | Deepen long CPU scans over nonzero-offset, capacity-padded packed K/V views; preserve raw decode parity and read-only packed cache views | CPU-only packed-view parity; no GPU timing or performance claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **331** | `tests/test_fuse_scorev.py` | Extend long-path score×V coverage to distinct Q/K and per-head V gradients while preserving raw scores × strict `tril(-1)` × V | CPU-only gradient parity; no GPU timing or performance claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **332** | `opt/docs-v70` | Refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #331 | Docs only; no GPU evidence | — |
| **333** | `tests/test_layout_v3.py` | Protect strided sampler outputs and neighboring backing values across multinomial, narrow top-k, and full-vocabulary fallback paths | CPU-only layout contract; no GPU timing or sampler-layout claim | **P0** GPU layout/measurement remains open |
| **334** | `tests/test_rope_fuse.py` | Deepen paired T=1 RoPE CPU fallback coverage with eager input-gradient parity | CPU-only gradient parity; no GPU timing or fused-RoPE result | **P0** GPU fused-RoPE validation remains open |
| **335** | `benchmarks/bench_generate.py` / `tests/test_bench_generate.py` | Reject malformed or empty AUTO prompt-length lists before model/device setup | CPU-only fail-closed harness contract; no GPU generate claim | **P1** GPU generate measurement remains open |
| **336** | `tests/test_cuda_build.py` | Keep non-`1` extension flags on the pure-Python path, including paired CUDA/CPU flags | CPU-only setup contract; no GPU build or timing claim | **P0** GPU build/measurement remains open |
| **337** | `tests/test_attn_bwd_v4_contract.py` | Cover fp16/bfloat16 tiled strict-tril backward parity and restore input-gradient dtypes | CPU-only half-dtype parity; no GPU timing or train claim | **P0** real GPU attention-backward measurement remains open |
| **338** | `tests/test_compile_v11_contract.py` | Verify the successful default `train_bwd` compile probe forwards mode without forcing `fullgraph=False` and cleans up | CPU-only compile contract; no GPU/CUDA-graph measurement | **P1** GPU inductor/CUDA-graph validation remains open |
| **339** | `tests/test_decode_gemm_v11.py` | Cover packed T=1 decode dtype parity for fp16/bfloat16 and shared/per-head value layouts | CPU-only decode parity; no GPU timing or performance claim | **P0** real GPU decode/cold CUDA–Triton validation remains open |
| **340** | `tests/test_bench_gpu_attn.py` | Keep timing-only backend failures as explicit skips with null medians and no CPU-smoke GPU claims | CPU-only GPU-harness contract; no GPU timing result | **P0** real GPU measurement remains open |
| **341** | `tests/test_dataloader.py` | Keep CPU DataLoader workers on `pin_memory=False` without CUDA probing or CUDA object construction | CPU-only worker/H2D contract; no H2D timing or overlap claim | **P1** GPU H2D overlap remains unmeasured |
| **342** | `tests/test_amp_train_contracts_v12.py` | Verify forward-only AMP sends flattened fp32 logits and int64 targets to cross-entropy outside autocast | CPU-safe AMP contract; no GPU throughput claim | **P3** GPU AMP train measurement remains open |
| **tip** | `tests/test_amp_train_contracts_v12.py` | Tip `59030a3`: #332–#342 add docs and CPU-safe layout, RoPE, gen-bench, CUDA-build, analytic-bwd, compile, decode, GPU-harness, DataLoader-worker, and forward-only AMP CE contracts only; no CUDA run or GPU evidence | **P0** real GPU measurement / cold CUDA–Triton validation remains open |
| **tip** | `OPT_NOTES.md` (profile-v20) | Retain matched CPU operator counts through `59030a3`: attention/forward/generate `copy_`=2/12/394 per call, `cat=0`, `contiguous=0`; #332–#342 add CPU-safe contract or docs coverage only | CPU-only profile evidence and contract coverage; no GPU timing or speedup claim | **P0** real GPU measurement / cold CUDA–Triton validation remains open
...
- **#277** `aeec930` refreshes the status/backlog docs through #275; docs-only.
- **#278** `abef868` adds CPU-safe AUTO threshold override coverage: explicit cold independence, then fallback to the later decode threshold when unset; no GPU timing claim.
- **#279** `8d4ea64` adds CPU-only score×V gradient parity for aliased self-attention Q/K across blocked and online paths; no GPU timing claim.
- **#280** `4de8f5a` adds CPU-only multi-query online decode coverage for signed raw scores and per-head past values; no GPU timing claim.
- **#281** `76e02e6` adds CPU-only strided sampler-output coverage for default and top-k decode narrows; no GPU timing claim.
...
- **#288** `945ba03` adds CPU-safe shared-V tiled decode autograd parity; no GPU timing.
- **#289** `460304c` adds CPU-safe AMP forward-only environment coverage; no GPU throughput claim.
- **#290** `2f09e47` keeps GPU-harness synchronization device-local and locks the CPU smoke contract; no CUDA run occurred here.
- **#291** `85f422a` adds global prefetch H2D opt-out coverage before CUDA probing or stream/event allocation.
- **#292** `125c8b7` adds CPU-only inclusive sparse guardrail threshold coverage; sparse remains opt-in with no GPU claim.

- **#293** `014a724` adds CPU-only batched blocked-tile strict-tril coverage at a non-divisible partial tile; no GPU performance claim.

- **#294** `8fedb1a` adds CPU-safe fail-closed AUTO prompt/threshold validation before model/device setup; no GPU performance claim.
...
- **#324** `2b39cbd` refreshes `OPT_STATUS.md` / `OPT_BACKLOG.md` through #323 as docs-v69; no GPU evidence.
- **#325** `6f0f57d` deepens CPU DataLoader H2D identity coverage and forbids CUDA probing/object construction on the CPU path; no H2D timing or overlap claim.
- **#326** `727f36f` covers the forward-only AMP train step, fp32 cross-entropy outside autocast, and post-step gradient clearing; no GPU throughput claim.
- **#327** `6569a51` extends blocked/prefill AUTO parity to shared and head-matched V layouts; no GPU performance claim.
- **#328** `0190637` hardens false-like `BDH_SPARSE_PROBE` values as default-off no-ops; no sparse or GPU claim.
- **#329** `b516f3b` covers malformed shared AUTO threshold recovery across decode and cold dispatch; no GPU timing or threshold claim.
- **#330** `eb0c2ee` covers nonzero-offset, capacity-padded packed online-decode views with raw parity and read-only cache assertions; no GPU timing or performance claim.
- **#331** `cc20049` extends long-path score×V coverage to distinct Q/K and per-head V gradients; no GPU timing or performance claim.
- **#332** `4dfab0e` refreshes `OPT_STATUS.md` / `OPT_BACKLOG.md` through #331 as docs-v70; no GPU evidence.
- **#333** `bf2758a` protects strided sampler outputs and neighboring backing values across CPU layout paths; no GPU timing or sampler-layout claim.
- **#334** `26e64c1` deepens paired T=1 RoPE CPU input-gradient parity against eager rotation; no GPU timing or fused-RoPE result.
- **#335** `a83f063` rejects malformed AUTO prompt-length lists before model setup; CPU harness contract only.
- **#336** `b967fe5` keeps non-1 extension flags on the pure-Python path; no GPU build or timing claim.
- **#337** `4c71e14` covers fp16/bfloat16 strict-tril analytic backward parity and input-gradient dtype restoration; no GPU train claim.
- **#338** `3e9dce4` covers default compile option forwarding without forcing `fullgraph=False`; no GPU/CUDA-graph measurement.
- **#339** `65c8ae2` covers packed T=1 decode dtype parity for fp16/bfloat16 and shared/per-head values; no GPU timing claim.
- **#340** `10cff45` keeps timing-only backend failures as explicit skips with null medians; CPU smoke makes no GPU timing claim.
- **#341** `c4c87a6` covers CPU DataLoader workers with pinning disabled and no CUDA setup; no H2D timing or overlap claim.
- **#342** `59030a3` covers forward-only AMP CE input boundaries: fp32 flattened logits, int64 flattened targets, and no autocast in CE; no GPU throughput claim.
- **Current tip** `59030a3` carries the flat profile-v20 counts plus CPU-only contracts/docs through #342; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
- **Current tip** `c4c87a6` carries the flat profile-v20 counts plus CPU-only contracts/docs through #341; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
