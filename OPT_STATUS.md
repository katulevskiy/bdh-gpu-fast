# OPT status — landed work (#1–#605; docs-v89)
...
| Compile | Off by default; invalid and normalized `BDH_COMPILE_PROBE` values fail closed to `train_bwd`, including missing-`example_y` training callers and missing-`example_x` no-probe callers; input-only forward probes succeed without targets; successful probes clear probe and pre-existing gradients and restore caller training state, eval probes run under no-grad and restore caller mode, while failed backward probes discard the wrapper, clear probe gradients, and preserve the eager module and caller mode; construction fallback preserves the eager module, caller mode, gradients, and compile options across both fullgraph settings | CUDA graphs/inductor unmeasured |
...
The retained profile-v20 baseline is `aten::copy_` 2/call for attention, 12/call for forward, and 394/call for generate, with `aten::cat=0` and `aten::contiguous=0`. The #403–#583 and #589–#605 additions are CPU-safe contracts, diagnostics, and docs; #450/#465/#479/#495/#512/#525/#540/#603 refine CUDA-build/device/runtime skip handoffs, exact build-flag handling, schema-v10 requested-config retention, stdout/JSON parity, preserved device-probe errors, and no-timing boundaries, while #552/#566/#582/#598 harden explicit CPU extension selection and #579 labels the CPU CUDA benchmark fallback. None adds CUDA timing, GPU throughput, sparse-kernel evidence, or cold CUDA–Triton validation. These are CPU call-count observations, not GPU performance claims.
