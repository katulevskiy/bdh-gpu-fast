...
=======
| **180** | `docs: refresh optimization matrix through amp-train-v4` | Refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #179 while preserving profile-v16 counts and the real-GPU P0 blocker | Docs only | — |
...
>>>>>>> 408d35d (docs: refresh optimization matrix through cache-bench-v3)
...
- **Current tip #181** `opt/cache-bench-v3` is at `f34d908`; adds CPU-only initial→final packed-cache footprint accounting with no GPU memory, timing, or speedup claim.
