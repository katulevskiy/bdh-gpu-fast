...
| **tip** | `test/cuda_build.py` | Deepen the CPU-safe missing-`nvcc` skip contract so a directory named `nvcc` remains a clear no-op before CUDAExtension setup | CPU-only setup/skip coverage; no GPU build or timing claim | **P0** real GPU measurement / cold CUDA-Triton validation remains open
...
- **Current tip** `d6a5455` carries the flat profile-v19 counts plus CPU-only structured-skip, prefetch-gate, AMP skip-reason, blocked/online gradient, sparse-probe, AUTO-environment, eager raw-score decode, explicit-backend AUTO-gate, top-k-one sampler-layout, distinct-Q/K score-V parity, blocked-RoPE tile, successful compile-backward cleanup, and CUDA missing-`nvcc` skip diagnostics; the matrix remains CPU/docs evidence only and the real-GPU P0 blocker is unchanged.
