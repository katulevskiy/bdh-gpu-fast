...
Compares cold-path ``tril(diagonal=-1)`` score×V backends on CUDA.
...
from kernels.cuda_attn import tril_score_v  # noqa: E402
...
def main() -> int:
    ...
    print(f"device={device}  gpu={gpu_name!r}  dtype={dtype}")
    print(f"shape B={B} H={H} T={T} N={N} D={D}")
    ...
    t_eager = _bench(eager_tril_attn, (Q, K, V), warmup=args.warmup, iters=args.iters)
    ...
    if device.type != "cuda":
        print(
            "NOTE: --force-cpu used; Triton kernel not executed; "
