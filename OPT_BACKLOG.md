...
| **P1** | **Decode GEMM / copy tax on generate** | **Host tax cut** #44+#48; **decode-mm** + **decode-online-v2** landed (CPU long-S blocked win). profile-v4: `bmm`~43% `mm`~30%; **cats=0**. Remaining = **GPU** measure. Default still eager. | A100/H100: `bench_generate.py` + `bench_gpu_attn.py --mode decode`; keep cat-free | Medium |
