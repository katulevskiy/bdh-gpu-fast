...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / profile-v17 source tip `f34d908` / profile-v18 source tip `c25aa9f` / current code tip `8b562f4` after profile-v20, #251 force-CPU GPU-harness coverage, #252 CPU prefetch-gate coverage, #253 docs refresh, #254 active-AMP-state coverage, #255 absent generate-impl environment cleanup, #256 batched blocked-tile gradient parity, #257 sparse density-only crossover coverage, #258 AUTO threshold fallback coverage, #259 distinct-Q/K score-V gradient coverage, #260 signed raw-score decode coverage, #262 sampler-layout overflow coverage, #263 mixed-dtype RoPE output coverage, #264 failed compile-probe cleanup, #265 CUDA_PATH missing-nvcc skip coverage, #266 strict-tril backward boundary coverage, #268 tiled packed per-head decode gradient coverage, #270 explicit prefetch H2D opt-out coverage, #271 run-level GPU skip status, #272 terminal sparse guardrail coverage, #273 blocked-tile raw-score coverage, #274 invalid gen-bench selection coverage, #275 invalid AMP state coverage, #277 docs refresh, #278 AUTO threshold override coverage, #279 aliased score-V gradient coverage, #280 multi-query online decode coverage, #281 strided sampler output coverage, #282 mixed-dtype RoPE coverage, #283 compile backward probe coverage, and #284 PATH missing-nvcc coverage. Profile-v20 remains flat versus profile-v19/v18/v17/v16: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. These later landings add only CPU-safe contract or docs coverage; none adds a CUDA run or GPU result. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
...
Current evidence boundary through `8b562f4`: #277 is docs-only; #278–#284 add CPU-safe AUTO, score×V gradient, multi-query decode, strided-sampler, mixed-dtype RoPE, compile-probe, and missing-nvcc contract coverage only. No CUDA run, GPU timing, GPU speedup, or cold CUDA–Triton validation was added. Keep the real-GPU measurement / cold CUDA–Triton validation blocker at P0.
...
| **P1** | **CUDA prefetch H2D overlap** | **Landed #92 + #139 + #203 + #233 + #270** `BDH_PREFETCH_H2D=1`: one pinned batch staged on a side stream with event handoff; #139 explicitly verifies that CPU is an identity/no-stream/event/device-lookahead no-op, and #203 gates unavailable CUDA before stream/event construction with actionable skip reasons; #233 also catches availability-probe `AssertionError`/`RuntimeError` before any stream/event allocation; #270 adds explicit `BDH_PREFETCH_H2D=0` no-probe coverage. No CPU throughput evidence or GPU overlap result exists. | CUDA host: measure H2D overlap, lifetime safety, and end-to-end train throughput; keep `BDH_PREFETCH_H2D=1` opt-in to measurement only | Env blocker |
...
| **P2** | **Sparsity follow-through** | **Landed #145 sparse-v3 + #237 + #272:** stable success/guardrail-failure exit outcomes and coverage/notes build on the gated density probe from `opt/sparse-v2`; `BDH_SPARSE_PROBE` is explicit opt-in, short-train x=26.63% / xy=11.37% at step 150, and the conservative x≥20% / xy≥8% guardrail passes. CPU sparse **never reliably beat dense** → dense/default path and sparse **stay OFF**; GPU sparse remains unmeasured; #237 only verifies the CPU density-only completion boundary. | GPU sparse bench only if density ≪10% and a kernel win is demonstrated | Speculative |
...
| **P3** | **Hardware / dtype** | **Landed `opt/bf16-train`+#54 `opt/amp-deepen`+#142 `opt/amp-train-v3`+#179 `opt/amp-train-v4`+#207:** opt-in `BDH_AMP_DTYPE` with explicit float32/bf16/fp16 configuration coverage, CPU-safe backend skips, and GradScaler gated to CUDA fp16; fp32 defaults remain unchanged. GPU train throughput still open; #275 adds CPU-only invalid-dtype state-preservation coverage. | GPU box microbench | Env |
