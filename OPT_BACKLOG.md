...
Post-#237 sparse density-only probe contract (`66c1680`): verify a passing density guardrail completes without starting the CPU sparse-vs-dense crossover sweep. The probe remains opt-in and production sparse wiring remains unchanged; CPU-only coverage with no GPU sparse claim.
...
| **P1** | **CUDA prefetch H2D overlap** | **Landed #92 + #139 + #203 + #233** `BDH_PREFETCH_H2D=1`: one pinned batch staged on a side stream with event handoff; #139 explicitly verifies that CPU is an identity/no-stream/event/device-lookahead no-op, and #203 gates unavailable CUDA before stream/event construction with actionable skip reasons; #233 also catches availability-probe `AssertionError`/`RuntimeError` before any stream/event allocation. No CPU throughput evidence or GPU overlap result exists. | CUDA host: measure H2D overlap, lifetime safety, and end-to-end train throughput; keep `BDH_PREFETCH_H2D=1` opt-in to measurement only | Env blocker |
