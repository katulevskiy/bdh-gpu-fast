...
| **P2** | **Fused RoPE kernel** | Attn: `mul`/`copy_` from strided rotate. **Landed #90** `opt/rope-fuse-v2` and **#124** `opt/rope-gpu-scaffold`: no expand/stack, table-pair T=1, zero-stride cis-row Triton scaffold, and blocked CPU fallback; default remains eager. GPU fused-RoPE validation is still open. | GPU Triton microbench still open | Low–medium |
...
**CPU honesty (this box / profile source `f10bdd4`; documented code tip `546192d`):**
