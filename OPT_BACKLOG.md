...
| **P1** | **CUDA prefetch H2D overlap** | **Landed #92 + #139 + #203** `BDH_PREFETCH_H2D=1`: one pinned batch staged on a side stream with event handoff; #139 explicitly verifies that CPU is an identity/no-stream/event/device-lookahead no-op, and #203 gates unavailable CUDA before stream/event construction with actionable skip reasons. No CPU throughput evidence or GPU overlap result exists. | CUDA host: measure H2D overlap, lifetime safety, and end-to-end train throughput; keep `BDH_PREFETCH_H2D=1` opt-in to measurement only | Env blocker |
