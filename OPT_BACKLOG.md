...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), with profile-v20 counts unchanged: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, `cat=0`, and `contiguous=0`. Through tip `8fedb1a`, #287–#294 and the prefetch H2D coverage add CPU-safe contract/harness coverage only; none adds a CUDA run, GPU timing, GPU speedup, or cold CUDA–Triton validation. Absolute ms are **profiler-inflated**; use `% self CPU` and call counts. Re-run on GPU before claiming kernel wins.
...
Current evidence boundary through `8fedb1a`: #285–#286 are docs-only; #287 adds raw strict-tril backward contract coverage, #288 adds shared-V tiled decode autograd parity, #289 adds CPU-safe AMP forward-only configuration coverage, #290 keeps GPU-harness synchronization device-local, #291 adds global prefetch H2D opt-out coverage, and #292 adds sparse guardrail threshold coverage, and #293 adds CPU-only batched partial-tile strict-tril coverage, and #294 adds CPU-safe fail-closed AUTO input validation. No CUDA run, GPU timing, GPU speedup, or cold CUDA–Triton validation was added. Keep the real-GPU measurement / cold CUDA–Triton validation blocker at P0.
