...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / current code tip `d0e667b` after #159 zerograd-v2, #160 cache-bench-v2, #161 profile-v16, and #163 structured GPU-measurement harness. Profile-v16 remains flat versus v15: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. #163 adds structured cold/decode/dtype measurement and CPU-safe skip coverage but no CUDA run or GPU result. Earlier context includes #143 docs, #144 profile-v14, #145 sparse probe exit codes, #146 cuda-cold-v4, #147 online-decode, #148 docs refresh, #149 AUTO threshold sweep smoke, #150 Triton cold-v4 skip diagnostics, #151 docs refresh, #152 generate copy-v2, #153 docs refresh, #154 rope-fuse-v3, #155 docs refresh, and #156 layout-v3. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
