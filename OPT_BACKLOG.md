...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / profile-v17 source tip `f34d908` / profile-v18 source tip `c25aa9f` / current code tip `b00dbd1` after profile-v20, #251 force-CPU GPU-harness coverage, #252 CPU prefetch-gate coverage, #253 docs refresh, #254 active-AMP-state coverage, #255 absent generate-impl environment cleanup, #257 sparse density-only crossover coverage, and #256 batched blocked-tile gradient parity. Profile-v20 remains flat versus profile-v19/v18/v17/v16: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. These later landings add only CPU-safe contract or docs coverage; none adds a CUDA run or GPU result. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
...
| Current tip | **Landed** at `b00dbd1` — profile-v20 counts remain 2/12/394 with `cat=0` and `contiguous=0`; later changes are CPU/docs contracts only, and the real-GPU P0 blocker remains open
