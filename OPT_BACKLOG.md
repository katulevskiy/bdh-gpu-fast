...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / profile-v17 source tip `f34d908` / profile-v18 source tip `c25aa9f` / current code tip `f4c7cab` after #202 docs refresh through #201, #203 unavailable-CUDA prefetch gating, #204 profile-v18, #205 zerograd logger lifecycle coverage, #206 docs refresh through #204, #207 CPU AMP failure-state coverage, #208 blocked-tile long-path gradient parity, #209 CPU-safe Triton RoPE skip diagnostics, #210 docs refresh through #209, #211 partial cache-page accounting, #212 CUDA cold shape validation, #213 Triton cold skip-gate coverage, #214 sparse guardrail terminal coverage, #215 generate-benchmark environment contracts, #216 AUTO threshold lower-bound coverage, #217 docs refresh through #215, #218 online decode raw-score contract coverage, #219 B=1 non-flat score×V view parity coverage, #220 B=1 score×V view alignment, #221 multi-step sampler layout contract coverage, #223 docs refresh through #221, #224 paired RoPE table-boundary coverage, #225 failed-backward compile-probe fallback coverage, #228 profile-v19, #229 strict-tril backward support coverage, #230 packed per-head decode autograd parity, #231 docs refresh through #230, #232 structured backend skip results, #233 unavailable-CUDA prefetch diagnostics, #234 CPU AMP skip-reason coverage, #235 long blocked/online gradient parity, #236 docs refresh through #235, #237 sparse density-only probe coverage, #238 generate-AUTO environment cleanup, and #239 eager raw-score decode coverage, and #240 explicit-backend AUTO-gate coverage, #241 top-k-one sampler-layout coverage, #242 distinct-Q/K score-V parity coverage, and #244 blocked RoPE tile contract coverage, #245 successful compile-backward probe cleanup, #247 shared-V strict-tril backward, #248 docs refresh, #249 CUDA-mirror packed decode gradients, and profile-v20. Profile-v20 remains flat versus profile-v19/v18/v17/v16: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. #236 is docs-only; #237 adds CPU-only density-only sparse-probe contract coverage; #238 adds CPU-only AUTO environment cleanup coverage; #239 adds CPU-only eager raw-score decode coverage; #240 adds CPU-only explicit-backend AUTO-gate coverage; #241 adds CPU-only top-k-one sampler-layout coverage; #242 adds CPU-only distinct-Q/K score-V parity coverage; #244 adds CPU-only blocked-RoPE tile validation; #245 adds CPU-only successful compile-backward cleanup coverage; #247 adds CPU-only shared-V strict-tril backward coverage; #248 is docs-only; #249 adds CPU-only CUDA-mirror packed decode gradient coverage; profile-v20 records unchanged counts; the current CUDA-build tip deepens the CPU-only missing-nvcc skip contract. None adds a CUDA run or GPU result. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
...
| Profile-v20 (current tip) | **Landed** at `f4c7cab` — matched CPU attention/forward/generate `copy_` counts remain 2/12/394 per call with `cat=0` and `contiguous=0`, flat versus profile-v19; no GPU timing or speedup claim
