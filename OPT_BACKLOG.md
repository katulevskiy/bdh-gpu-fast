...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / profile-v17 source tip `f34d908` / profile-v18 source tip `c25aa9f` / current code tip `ff5b071` after #202 docs refresh through #201, #203 unavailable-CUDA prefetch gating, #204 profile-v18, #205 zerograd logger lifecycle coverage, #206 docs refresh through #204, #207 CPU AMP failure-state coverage, #208 blocked-tile long-path gradient parity, #209 CPU-safe Triton RoPE skip diagnostics, #210 docs refresh through #209, #211 partial cache-page accounting, #212 CUDA cold shape validation, #213 Triton cold skip-gate coverage, #214 sparse guardrail terminal coverage, #215 generate-benchmark environment contracts, #216 AUTO threshold lower-bound coverage, #217 docs refresh through #215, #218 online decode raw-score contract coverage, #219 B=1 non-flat score×V view parity coverage, #220 B=1 score×V view alignment, #221 multi-step sampler layout contract coverage, #223 docs refresh through #221, #224 paired RoPE table-boundary coverage, #225 failed-backward compile-probe fallback coverage, #228 profile-v19, #229 strict-tril backward support coverage, and #230 packed per-head decode autograd parity, #231 docs refresh through #230, #232 structured backend skip results, and #233 unavailable-CUDA prefetch diagnostics, and #234 CPU AMP skip-reason coverage, and #235 long blocked/online gradient parity. Profile-v19 remains flat versus v17/v16: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. #202, #206, #210, and #217 are docs-only; #203 adds CPU-only H2D skip diagnostics before CUDA construction; #204 adds matched CPU-only profile evidence; #205 adds CPU-only logger lifecycle coverage; #207 adds CPU-only AMP failure-state coverage; #208 adds CPU-only long-path forward/gradient parity; #209 adds CPU-only RoPE skip-diagnostic coverage; #211 adds CPU-only partial-page accounting; #212 adds CPU-only cold-shape validation; #213 adds CPU-only Triton skip-gate coverage; #214 adds CPU-only sparse guardrail terminal coverage; #215 adds CPU-only generate-benchmark environment-contract coverage; #216 adds CPU-only AUTO threshold lower-bound coverage; #217 is docs-only; #218 adds CPU-only raw-score decode contract coverage; #219 and #220 add CPU-only B=1 score×V view coverage; #221 adds CPU-only multi-step sampler-layout coverage; #224 adds CPU-only paired RoPE table-boundary coverage; #225 adds CPU-only failed-backward compile-probe fallback coverage; #228 adds matched CPU-only profile-v19 evidence; #229 adds CPU-only strict-tril backward support coverage; and #230 adds CPU-only packed per-head decode autograd parity coverage; #231 is docs-only; #232 adds CPU-only structured backend skip/result coverage; and #233 adds CPU-only unavailable-CUDA prefetch probe-error coverage; #234 adds CPU-only unavailable-AMP skip-reason coverage; #235 adds CPU-only long blocked/online gradient parity coverage. None adds a CUDA run or GPU result. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
