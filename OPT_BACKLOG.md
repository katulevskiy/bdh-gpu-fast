...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / current code tip `86315f0` after #143 docs, #144 profile-v14, #145 sparse probe exit codes, #146 cuda-cold-v4, #147 online-decode, #148 docs refresh, #149 AUTO threshold sweep smoke, #150 Triton cold-v4 skip diagnostics, #151 docs refresh, #152 generate copy-v2, #153 docs refresh, #154 rope-fuse-v3, #155 docs refresh, and #156 layout-v3; profile-v15 records attention `bmm` 31.99% / `mul` 22.22% / `complex` 19.12% / `copy_` 8.87%, forward `bmm` 29.05% / `mm` 23.15% / `mul` 15.99% / `complex` 11.42% / `copy_` 7.05%, and generate `mm` 21.81% / `bmm` 14.15% / `copy_` 0.86%; counts remain `copy_`=2/call attention, 12/call forward, 394/call generate, with `cat=0` and `contiguous=0`. The separate #156 CPU `torch.profiler` probe records one intentional eval encoder-cache materialization, two warm eager clone-backed shapes per layer, and ATen sampler contiguous shapes `(B,V)` once plus `(B,)` per sampled token; neither landing changes defaults or provides GPU evidence. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
...
| **P2** | **Fused RoPE kernel** | Attn: `mul`/`copy_` from strided rotate. **Landed #90**, **#124**, and **#138**: no expand/stack, table-pair T=1, zero-stride cis-row Triton scaffold, conservative complete-tensor/device/dtype skip gates, and blocked/PyTorch CPU fallback; default remains eager. GPU fused-RoPE validation is still open. | GPU Triton microbench still open | Low–medium |
