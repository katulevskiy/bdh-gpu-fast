...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / profile-v17 source tip `f34d908` / profile-v18 source tip `c25aa9f` / current code tip `5995a7c` after #202 docs refresh through #201, #203 unavailable-CUDA prefetch gating, and #204 profile-v18. Profile-v18 remains flat versus v17/v16: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. #202 is docs-only; #203 adds CPU-only H2D skip diagnostics before CUDA construction; #204 adds matched CPU-only profile evidence. None adds a CUDA run or GPU result. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
