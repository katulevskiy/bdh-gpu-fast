...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v15 source tip `86315f0` / profile-v16 source tip `717c38e` / profile-v17 source tip `f34d908` / profile-v18 source tip `c25aa9f` / current code tip `a48b7e1` after #202 docs refresh through #201, #203 unavailable-CUDA prefetch gating, #204 profile-v18, #205 zerograd logger lifecycle coverage, #206 docs refresh through #204, #207 CPU AMP failure-state coverage, #208 blocked-tile long-path gradient parity, and #209 CPU-safe Triton RoPE skip diagnostics. Profile-v18 remains flat versus v17/v16: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, with `cat=0` and `contiguous=0`. #202 and #206 are docs-only; #203 adds CPU-only H2D skip diagnostics before CUDA construction; #204 adds matched CPU-only profile evidence; #205 adds CPU-only logger lifecycle coverage; #207 adds CPU-only AMP failure-state coverage; #208 adds CPU-only long-path forward/gradient parity; #209 adds CPU-only RoPE skip-diagnostic coverage. None adds a CUDA run or GPU result. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
