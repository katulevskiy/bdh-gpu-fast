...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v14 source tip `68f949a` / current code tip `4e01e38` after #143 docs, #144 profile-v14, #145 sparse probe exit codes, #147 online-decode, and #146 cuda-cold-v4; profile-v14 is flat versus profile-v13 at `copy_`=2/call attention, 12/call forward, 394/call generate, with `cat=0` and `contiguous=0`; profile-v13 source tip `45b4afe`; #141 blocked-tile-v3; #142 amp-train-v3; cfg `layers=4 d=128 nh=4 B=4 T=128`, generate prompt=16 / new=32. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
