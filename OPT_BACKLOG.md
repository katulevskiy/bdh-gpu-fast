...
| **P1** | **`torch.compile` / inductor** | Forward: `copy_` ~23%, `mul` ~16%, LN ~9%, ReLU ~7%. **train-fuse** + **compile-harden** (train probe, graph-break docs, CPU inductor parity tests) landed; measure on GPU. | GPU compile parity / bench | Low |
