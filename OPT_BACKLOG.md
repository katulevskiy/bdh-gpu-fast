...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), profile-v14 source tip `68f949a` / current code tip `6d2dff9` after #143 docs, #144 profile-v14, #145 sparse probe exit codes, #146 cuda-cold-v4, #147 online-decode, #148 docs refresh, #149 AUTO threshold sweep smoke, #150 Triton cold-v4 skip diagnostics, #151 docs refresh, #152 generate copy-v2, #153 docs refresh, and #154 rope-fuse-v3; profile-v14 is flat versus profile-v13 at `copy_`=2/call attention, 12/call forward, 394/call generate, with `cat=0` and `contiguous=0`; profile-v13 source tip `45b4afe`; #141 blocked-tile-v3; #142 amp-train-v3; cfg `layers=4 d=128 nh=4 B=4 T=128`, generate prompt=16 / new=32. Absolute ms are **profiler-inflated**; use **% self CPU** and call counts. Re-run on GPU before claiming kernel wins.
...
| **P1** | **Decode GEMM / copy tax on generate** | **Host tax cut** #44+#48; **decode-gemm-v2** preserves packed KR/V strides in T=1 Triton decode without per-step contiguous staging, and #131 adds the CPU AUTO threshold sweep, #149 adds CPU-safe dispatcher smoke, #152 confirms the remaining CPU-safe copy ceiling, #153 is docs-only, and #154 reuses the warmed T>1 RoPE narrow without GPU evidence; #135 deepens the B=1 T=1 shared-V epilogue alongside **decode-mm** + **decode-online-v2** + **attn-auto** + **triton-decode-v3** + **cuda-decode-v3** + **`opt/prefill-blocked`** + **`opt/auto-tune`**: AUTO long-T cold+decode (adaptive blocked cold @T≥256), with optional independent cold threshold. CPU e2e AUTO **1.26×@1024 / 1.39×@2048**; IMPL=blocked ~1.23–1.43×; short A/B ~1.25× @1024. Remaining = **GPU** measure / re-tune thr. Default still eager. | A100/H100: `bench_generate.py --mode auto-ab` + `--mode impls` + `bench_gpu_attn.py --mode decode`; keep cat-free | Medium |
