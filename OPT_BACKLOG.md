...
|---|------|----------------------------|--------|------|
| **P0** | **Measure Triton/CUDA fused tril-score×V on real GPU** | Default **eager** still (**GPU blocker**; profile-v9, #100, and #102 are CPU evidence only): attention `bmm` 30.50% / `mul` 30.78% / `copy_` 19.45%; forward `bmm` 34.12% / `mm` 24.93% / `mul` 17.69% / `copy_` 9.09%. #96–#102 add no GPU timing or speedup claim; #95 isolated forward `copy_`=24→18, while profile-v9 warmed harness is 16/call and generate is 558/call before #102. #100 lowers blocked/online inference epilogue buffering and tightens T=1 decode tiling; #102 cuts generate `copy_` ~558→~398 and keeps `cat` at 0, but GPU validation remains open. #93 remains CPU-only blocked-path evidence. | A100/H100: `bench_gpu_attn.py` (+ fused score×V) | Env blocker |
