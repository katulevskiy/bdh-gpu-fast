...
Profile source: `benchmarks/profile_forward.py` on CPU (`torch 2.14.0+cu130`, `cuda=False`), with profile-v20 counts unchanged: attention `copy_`=2/call, forward `copy_`=12/call, generate `copy_`=394/call, `cat=0`, and `contiguous=0`. Through tip `d915d57`, #307 prefetch, #308 docs-v67, #309 sparse, #310 AMP, #311 blocked-prefill, #312 AUTO validation, #314 packed shared-V online-decode parity, #315 per-head score×V parity, #316 layout, #317 docs-v68, #318 RoPE, #319 compile, #320 CUDA-build, #321 duplicate-Q backward, #322 forced-CPU measurement, #323 packed decode GEMM dtype parity, and the gen-bench threshold-sweep follow-ups add CPU-safe contract/harness/docs coverage only; none adds a CUDA run, GPU timing, GPU speedup, sparse-kernel result, or cold CUDA–Triton validation. Absolute ms are **profiler-inflated**; use `% self CPU` and call counts. Re-run on GPU before claiming kernel wins.
Post-#307–#323+ current tip (`d915d57`): #307 deepens CPU H2D-prefetch identity/no-allocation behavior; #308 is docs-v67 through #306; #309 makes the sparse density guardrail evaluate the latest sample; #310 completes CPU-safe AMP forward-only toggle coverage and explicit override precedence; #311 exercises blocked/prefill AUTO dispatch above threshold with non-divisible strict-tril parity; #312 rejects malformed/negative AUTO cold thresholds before dispatch with clean recovery after a valid update; #314 preserves packed shared-V online decode parity across a tiled multi-query boundary; #315 preserves distinct-Q/K score×V parity with per-head V values across the long blocked/online path; #316 covers non-unit-stride sampler outputs; #317 is docs-v68; #318 covers paired T=1 RoPE writes into a strided output slot; #319 covers successful compile option forwarding and probe cleanup; #320 makes the explicit CPU extension flag take precedence over CUDA build; #321 pins duplicate-Q strict-tril backward locality; and #322 keeps forced-CPU smoke from initializing GPU metadata while preserving CPU timing labels; #323 adds packed multi-query decode GEMM float16/bfloat16 parity across broadcast and per-head values. The gen-bench threshold-sweep follow-up de-duplicates decode thresholds and mirrors the cold gate. All evidence is CPU-only; sparse remains opt-in/default-off, defaults remain eager/fp32/AUTO-off, and real GPU measurement plus cold CUDA–Triton validation remain P0.
...
Post-#190 docs refresh (`a618356`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #188 while preserving the profile-v17 counts and the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#191 rope-fuse-v4 (`39fe7f7`): keep paired and flat T=1 RoPE cache narrows synchronized when positions switch, preventing a stale flat narrow after a pairs-only lookup. Add CPU coverage for T>1 narrow invalidation and paired-cache rebuilds; eager RoPE and raw scores × strict `tril(diagonal=-1)` attention remain unchanged. No GPU timing, hardware kernel result, or RoPE win claim is recorded.

Post-#193 docs refresh (`4bd410a`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #191 while preserving profile-v17 counts and the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#194 B=1 shared-V epilogue coverage (`4fda79f`): add CPU coverage for packed-cache `out=` accumulation and the grad-safe fallback on the B=1 score×V path. Eager defaults, raw scores × strict `tril(diagonal=-1)`, and cat-free behavior remain unchanged; no GPU timing or speedup claim was added.

Post-#195 compile-v5 (`4ae8ab2`): make missing-target and first-probe `torch.compile` soft-fallback diagnostics explicit, add a CPU eval-probe smoke that restores the caller training mode, and preserve `BDH_COMPILE=0`, `BDH_COMPILE_PROBE=train_bwd`, eager attention, and the CPU-only/no-GPU claim.

Post-#196 cuda-build-v3 (`4e9d3e4`): check `CUDA_HOME`, `CUDA_PATH`, and PATH before constructing `CUDAExtension`, making missing-nvcc setup a clear CPU-safe no-op while retaining explicit CPU-only setup smoke. No GPU build, timing, or kernel claim was added; real GPU measurement and cold CUDA/Triton validation remain P0 blockers.

Post-#197 docs refresh (`ccadc3a`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #196 while preserving profile-v17 counts and the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#198 attn-bwd-v4 (`9ad9015`): add CPU-only T=1 strict-tril backward contract coverage across eager, blocked, online, Triton fallback, and CUDA-reference dispatch for shared-V and full-head-V layouts, asserting exact zero output and Q/K/V gradients when no past key exists. No GPU timing, hardware kernel result, or speedup claim was added; real GPU measurement and cold CUDA/Triton validation remain P0 blockers.

Post-#199 packed per-head decode GEMM parity (`269eb2d`): add CPU-only coverage for T=1 packed, capacity-strided per-head K/V views at and just above the decode oneshot boundary, locking the flattening shape/stride and view-preserving contract. Eager defaults and raw scores × strict `tril(diagonal=-1)` semantics remain unchanged; no GPU timing or performance claim was added, so real GPU decode measurement and cold CUDA/Triton validation remain P0 blockers.

Post-#200 docs refresh (`e253bba`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #198 while preserving profile-v17 counts and the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#201 CPU smoke timing scope (`c25aa9f`): distinguish real CUDA summaries from `--force-cpu` smoke output in schema version 2, reporting the actual device, timing scope, and structured reason. CPU contract coverage is explicit; forced-CPU medians are not GPU measurements, and real GPU measurement plus cold CUDA/Triton validation remain P0 blockers.

Post-#202 docs refresh (`3d3d1fe`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #201 while preserving the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#203 prefetch-v4 (`9669e06`): gate opt-in H2D staging on both CUDA device type and live runtime availability before constructing streams or events. CPU reports `device-not-cuda: cpu`; an unavailable CUDA runtime reports `CUDA unavailable: torch.cuda.is_available() is false`. Focused coverage is CPU-only and preserves the default `BDH_PREFETCH_ASYNC=1` / `BDH_PREFETCH_H2D=1`; no H2D timing, correctness, or overlap claim is added.

Post-#204 profile-v18 (`5995a7c`): re-profile the #199–#201 tip with two warmups, one wait, and three active CPU steps. Attention `copy_`=2/call, forward `copy_`=12/call, and generate `copy_`=394/call remain unchanged versus profile-v17, with `cat=0` and `contiguous=0`; the honest result is flat CPU evidence, not timing or speedup. Real GPU measurement and cold CUDA/Triton validation remain P0 blockers.

Post-#205 zerograd logger lifecycle (`e8ad099`): lock CPU-safe `TrainLossLogger.close()` to flush a partial window exactly once and reject updates after close. CPU lifecycle contract only; no GPU timing or speedup claim.

Post-#206 docs refresh (`6e0f313`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #204 while preserving profile-v18 counts and the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#207 CPU AMP failure-state contract (`dd11718`): verify an unavailable CPU AMP request reports an actionable error while preserving the prior dtype, PyTorch dtype, context, scaler, scaler-use, and forward-only state. CPU-only contract coverage; fp32/AMP-off defaults and GPU-only throughput claims remain unchanged.

Post-#208 blocked-tile long-path gradient parity (`dd4fc90`): extend CPU cold-path coverage to exact forward and Q/K/V gradient parity at `T=257` across shared and head-matched V layouts, while preserving strict raw `tril(-1)` behavior and eager defaults. No GPU timing or speedup claim was added.

Post-#209 Triton RoPE skip-reason contract (`a48b7e1`): validate the primary input type before probing optional Triton runtime state so invalid inputs receive a stable `v-not-tensor` diagnostic. CPU-safe skip coverage only; no GPU timing or fused-RoPE result was added.

Post-#210 docs refresh (`fecfba4`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #209. Docs-only; no code or GPU evidence.

Post-#211 partial cache-page accounting (`d3a37b7`): exercise a non-page-aligned final page in the linear cache-page benchmark and require the closed-form grow/copy-byte accounting to match the actual CPU run. Cache, generate, and defaults are unchanged; no GPU timing, memory, or speedup claim was added.

Post-#212 CUDA cold shape contract (`61af5e0`): validate malformed cold-attention shapes before optional native dispatch, yielding stable CPU-safe `ValueError` contracts without changing strict raw `tril(diagonal=-1)` math or defaults. No GPU timing or kernel claim was added.

Post-#213 Triton cold skip-gate contract (`bd809b9`): deterministically cover import-unavailable, CUDA-unavailable, and available import/device branches in `triton_cold_skip_reason()` without allocating CUDA tensors or launching kernels. CPU-safe diagnostics only; no GPU timing or fused-kernel result was added.

Post-#214 sparse guardrail terminal contract (`8ec4b16`): require an enforced density-guardrail failure to stop before CPU sparse-vs-dense crossover work. This is CPU-only contract coverage; sparse remains opt-in/default-off and no attention, GPU timing, or sparse-kernel claim was added.

Post-#215 generate-benchmark environment contracts (`eafe65c`): verify interrupted benchmark dispatch and AUTO threshold scopes restore their environment contracts. CPU-only test coverage; no GPU timing or generate-performance claim was added.

Post-#216 AUTO threshold lower-bound coverage (`265b25c`): reject negative `BDH_ATTN_AUTO_THRESHOLD` and `BDH_ATTN_AUTO_COLD_THRESHOLD` values before dispatch. CPU-only contract coverage; eager/AUTO-off defaults remain unchanged, and no GPU timing or threshold-performance claim was added.

Post-#217 docs refresh (`fb80c34`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #215 while preserving profile-v18 counts and the real-GPU P0 blocker. Docs-only; no code or GPU evidence.

Post-#218 online decode score contract (`50d40f5`): lock blocked, online, Triton-fallback, and CUDA-reference decode to raw QK scores without softmax, scale, or a fused attention op. CPU-only contract coverage; no GPU timing, correctness, or speedup claim was added.

Post-#219 B=1 non-flat score×V view coverage (`d218150`): preserve the 4-D packed-K path for non-flat B=1 views without staging and match eager decode. CPU-only parity/stride coverage; no GPU timing, correctness, or speedup claim was added.

Post-#220 B=1 score×V view alignment (`f344494`): match the non-flat K-view contract to the tiled `bmm` path while preserving eager decode parity. CPU-only shape/parity coverage; no GPU timing, correctness, or speedup claim was added.

Post-#221 multi-step sampler layout contract (`39ca536`): generate two tokens and require one `(B,)` sampler result per token while preserving `aten::cat=0` and avoiding `(B,V)` sampler materialization. CPU-only profiler contract coverage; no GPU timing or performance claim was added.

Post-#224 paired RoPE table-boundary contract (`ad7a50b`): guard paired T=1 RoPE lookups at negative and out-of-range table positions and verify rejected lookups do not poison the last valid flat or paired narrow cache. CPU-only boundary/skip coverage; no GPU timing, kernel result, or speedup claim was added.

Post-#225 failed-backward compile-probe fallback (`258b24c`): clear partial parameter gradients when the `train_bwd` probe fails before returning the eager fallback, preserve caller training mode, and lock the behavior with a synthetic CPU-only regression test. No GPU compile or performance claim was added.

Post-#228 profile-v19 (`1ad6b01`): re-run the matched CPU profile after #227 with two warmups, one wait, and three active steps. Attention/forward/generate remain `copy_`=2/12/394 per call with `cat=0` and `contiguous=0`; the honest result is flat versus profile-v18, with no GPU timing or CPU-to-GPU claim.

Post-#229 strict-tril backward support (`26d0b4f`): deepen CPU-only backward coverage so a single query loss reaches only its matching Q row and strict-past K/V rows across eager, blocked, online, Triton fallback, and CUDA-reference dispatch with shared-V and full-head-V layouts. Raw scores × `tril(diagonal=-1)` remains unchanged; no GPU timing or performance claim was added.

Post-#230 packed per-head decode GEMM parity (`0a42a67`): add CPU-safe autograd parity for capacity-strided per-head K/V decode GEMMs at the tiled oneshot boundary across eager, blocked, online, and Triton-fallback dispatch. Outputs and gradients match eager; no GPU timing or performance claim was added.

Post-#232 structured backend skip results (`efbb1f3`): bump the GPU-attention summary to schema v3, record explicit run/backend skip objects, mark each backend result with status/reason, and keep `median_ms` null when a backend is unavailable. CPU-only smoke/contract coverage; no GPU timing or speedup claim was added.

Post-#233 prefetch-v5 CUDA skip diagnostics (`ff5b071`): catch `AssertionError`/`RuntimeError` from `torch.cuda.is_available()` before constructing streams or events, preserving the CPU identity and unavailable-runtime no-allocation contracts. CPU-only validation; no CUDA H2D correctness, overlap, timing, or speedup claim was added.

Post-#234 CPU AMP skip-reason contract (`1f1edd9`): parameterize unavailable bfloat16 and float16 configuration requests and assert that skip outcomes retain the requested dtype and actionable backend context. CPU-only validation; no GPU timing or throughput claim was added.

Post-#235 long blocked/online gradient parity (`ee586d0`): extend the T=257 CPU forward/gradient parity case to both blocked and online tile aliases for shared and head-matched V layouts. Test-only CPU coverage; no GPU timing or performance claim was added.

Post-#236 docs refresh (`02bcf68`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #235 while preserving profile-v19 counts and the real-GPU/cold CUDA-Triton P0 boundary. Docs-only; no code or GPU evidence.

Post-#237 sparse density-only probe contract (`66c1680`): verify a passing density guardrail completes without starting the CPU sparse-vs-dense crossover sweep. The probe remains opt-in and production sparse wiring remains unchanged; #272 makes enforced guardrail failure terminal before CPU crossover work. CPU-only coverage with no GPU sparse claim.

Post-#238 generate-AUTO environment cleanup (`95ecae0`): verify interrupted AUTO runs restore `BDH_ATTN_AUTO`, `BDH_ATTN_AUTO_THRESHOLD`, and `BDH_ATTN_AUTO_COLD_THRESHOLD` when they were initially unset. CPU-only environment-contract coverage; no GPU timing or generate-performance claim was added.

Post-#239 eager raw-score decode contract (`34a0d9f`): include eager in the decode parity contract and retain raw QK scores with no softmax or scale across eager, blocked, online, Triton-fallback, and CUDA-reference dispatch. CPU-only contract coverage; no GPU timing or performance claim was added.

Post-#240 AUTO cold explicit-backend contract (`2a004c3`): verify explicit backends remain authoritative on both the AUTO cold and decode gates. CPU-only contract coverage; eager/AUTO-off defaults remain unchanged, with no GPU timing or performance claim added.

Post-#251/#252/#253 (`84bd3f6` / `0baa35d` / `53491cf`): keep forced-CPU GPU-attention smoke on CPU even when CUDA is available, short-circuit the CPU H2D gate before querying CUDA availability, and refresh the matrix through profile-v20. These are CPU contract/docs changes only; no GPU timing or speedup evidence was added.

Post-#254 AMP failure-state coverage (`94a4e55`): verify an unavailable CPU float16 switch cannot discard an already active AMP configuration, including dtype, PyTorch dtype, autocast context, scaler, scaler-use, and forward-only state. CPU-only failure-state coverage; no GPU timing or throughput claim was added.

Post-#255 generate environment cleanup (`1b67d13`): verify an interrupted generate-implementation sweep restores a dispatch environment variable that was initially absent. CPU-only environment-contract coverage; no GPU timing or generate-performance claim was added.

Post-#257 sparse density boundary (`042a01d`): cover the density-only sparse-probe crossover boundary while keeping the probe opt-in and production sparse wiring unchanged. CPU-only sparse contract coverage; sparse remains OFF and no GPU claim was added.

Post-#256 batched blocked-tile gradient parity (`b00dbd1`): extend the long blocked/online CPU parity matrix to `B=2` while retaining shared and head-matched wide-head V layouts. No GPU timing or performance claim was added; real GPU measurement and cold CUDA/Triton validation remain P0 blockers.

Post-#258 AUTO threshold fallback (`6abd53b`): verify that an unset `BDH_ATTN_AUTO_COLD_THRESHOLD` tracks later changes to `BDH_ATTN_AUTO_THRESHOLD` across both cold and decode resolvers. CPU-only AUTO contract coverage; eager/AUTO-off defaults remain unchanged and no GPU timing or threshold-performance claim was added.

Post-#259 distinct-Q/K score-V gradients (`f5a9cea`): verify blocked and online score×V preserve output and Q/K/V gradients for non-aliased query/key tensors. CPU-only gradient parity; no GPU timing or performance claim was added.

Post-#260 signed raw-score decode (`9b6d0a8`): cover signed QK scores and independent per-head values across eager, blocked, online, Triton-fallback, and CUDA-reference dispatch while preserving strict past-only raw score×V semantics. CPU-only decode contract coverage; no GPU timing or performance claim was added.

Post-#262 sampler-layout overflow (`1384ca5`): extend the CPU sampler-layout contract through the `top_k` greater-than-vocabulary clamp boundary without changing attention semantics, sampler behavior, or defaults. CPU-only coverage; no GPU timing or performance claim was added.

Post-#263 mixed-dtype RoPE output (`7c0e969`): cover fp16 RoPE math written into fp32 cache-style output slots across CPU entrypoints and sequence shapes. CPU-only output/parity coverage; no GPU timing or fused-RoPE result was added.

Post-#264 failed compile-probe eval cleanup (`3747356`): verify a failing `train_bwd` compile probe restores an eval caller mode and clears partial parameter gradients before retaining the eager module. CPU-only fallback coverage; no GPU compile or performance claim was added.

Post-#265 CUDA_PATH missing-nvcc skip contract (`6d6aaab`): cover a stale non-executable `CUDA_PATH` `nvcc` stub as a clear CPU-safe no-op before `CUDAExtension` setup, preserving the actionable missing-`nvcc` diagnostic and CPU references. No GPU build, timing, or kernel claim was added.

Post-#266 strict-tril backward boundary queries (`cbf0831`): extend the CPU-only locality contract to first, interior, and final query rows across supported dispatches and shared/head-matched V layouts. The test-only deepening reported 90 relevant tests passed; no GPU timing or training claim was added.

Post-#268 tiled packed per-head decode gradients (`aebc449`): cover CPU autograd parity for tiled T=1 decode GEMMs with capacity-strided packed per-head K/V views across blocked, online, and Triton-fallback paths. No CUDA timing or decode performance claim was added.

Post-#270 explicit prefetch H2D opt-out (`dae2e07`): cover `BDH_PREFETCH_H2D=0` as a CPU-safe no-probe path, including a configured CUDA device that must not trigger CUDA availability checks or stream/event allocation. Defaults and runtime behavior remain unchanged; no H2D timing or overlap claim was added.

Post-#271 run-level GPU skips (`75597c9`): mark unavailable-device run summaries with `status: skip` under schema v4. This clarifies skip reporting only; no real GPU timing or speedup evidence was added.

Post-#272 sparse guardrail terminal contract (`b9bf6c8`): make an enforced density-guardrail failure terminal before CPU crossover work. The sparse probe remains opt-in/default-off; CPU-only contract coverage adds no GPU sparse result.

Post-#273 blocked-tile raw-score contract (`3b3c082`): add deterministic CPU coverage at the tile boundary for blocked and online attention, asserting raw `Q @ K.T` scores multiplied by `tril(diagonal=-1)` and then `V`. No implementation rewrite, GPU timing, or performance claim was added.

Post-#274 invalid gen-bench selection (`81e0005`): fail closed on unknown implementation selections and selections without `eager` before model or device setup. The existing raw-score strict-tril, eager-reference, token-parity, and cat-free contracts remain unchanged; CPU validation only, with no GPU performance claim.

Post-#275 invalid AMP state (`915fe71`): preserve dtype, autocast context, scaler, and forward-only state when an invalid `float64` configuration is rejected. CPU-only failure-state coverage; no GPU timing or throughput claim was added.

Post-#277 docs refresh (`aeec930`): refresh `OPT_STATUS.md` / `OPT_BACKLOG.md` through #275. Docs-only; no code or GPU evidence.

Post-#278 AUTO threshold override contract (`abef868`): verify an explicit cold threshold remains independent from later decode-threshold changes, then follows the later decode threshold when the override is removed. CPU-safe contract coverage only; no GPU timing or threshold/tile claim.

Post-#279 aliased score-V gradients (`8d4ea64`): verify blocked and online score×V output plus Q/V gradients match eager when self-attention aliases Q and K. CPU-only gradient parity; no GPU timing or performance claim.

Post-#280 multi-query online decode (`4de8f5a`): cover multiple query rows with signed raw QK scores and independent per-head past values across eager, blocked, online, Triton-fallback, and CUDA-reference dispatch. CPU-only decode contract; no GPU timing or performance claim.

Post-#281 strided sampler output (`76e02e6`): cover default and top-k sampler writes into a non-contiguous one-column decode narrow and compare with the reference output. CPU-only layout contract; no GPU timing or performance claim.

Post-#282 mixed-dtype RoPE (`b573e96`): cover fp16 RoPE writes into non-contiguous fp32 cache slots across CPU entrypoints at T=1 and T=7, preserving the untouched backing-lane sentinel. CPU-only mixed-dtype parity; no GPU timing or fused-RoPE claim.

Post-#283 compile backward probe (`de72dad`): cover a `train_bwd` probe returning no loss, requiring clean gradient state and restoration of the caller's eval mode before the eager fallback. CPU-only compile contract; no GPU or CUDA-graph claim.

Post-#287 raw strict-tril backward contract (`609973c`): add deterministic CPU-only coverage across eager, blocked, online, Triton fallback, and CUDA-reference dispatch names. The contract preserves raw score×V with `tril(diagonal=-1)` and does not add GPU timing or speedup evidence.

Post-#288 shared-V tiled decode gradients (`945ba03`): add CPU-safe autograd parity for capacity-strided shared-V T=1 decode tiles with B>1 across blocked, online, and Triton CPU fallback paths. No GPU decode timing or performance claim was added.

Post-#289 AMP forward-only environment contract (`460304c`): verify CPU-safe `BDH_AMP_FORWARD_ONLY=1` defaults and explicit override behavior. This is test-only configuration coverage; no GPU throughput claim was added.

Post-#290 GPU-harness synchronization (`2f09e47`): synchronize only the device being measured and lock the CPU smoke contract. The box has no CUDA, so no GPU timing result was produced and the real-GPU P0 blocker remains open.

Post-#291 global prefetch H2D opt-out (`85f422a`): extend the CPU-safe contract to the global `USE_PREFETCH_H2D=False` setting, returning before CUDA probing or stream/event allocation for a configured CUDA device. GPU H2D overlap and throughput remain unmeasured.

Post-#292 sparse guardrail threshold boundary (`125c8b7`): cover inclusive `min_final_x` and `min_final_xy` floors while density-only mode remains crossover-free. Sparse stays opt-in/default-off; no GPU performance claim was added.

Post-#293 batched blocked-tile contract (`014a724`): deepen CPU-only strict-tril raw-score coverage for broadcast and head-matched V with a non-divisible final tile (`T=7`, `block_size=3`). No GPU performance claim was added.

Post-#294 gen-bench AUTO input contract (`8fedb1a`): add CPU-safe validation for invalid AUTO prompt and threshold inputs, failing closed before model/device setup. No GPU performance claim was added.

Post-#295 AUTO blank cold-threshold contract (`cde4388`): cover empty and whitespace-only `BDH_ATTN_AUTO_COLD_THRESHOLD` fallbacks to the live decode threshold, including strict-equality and above-threshold cold dispatch after a live decode-threshold update. No GPU performance claim was added.

Post-#297 tiled multi-query online decode contract (`1b4e695`): add CPU-only direct online-decode coverage across a decode score-tile boundary with signed raw QK scores and past-only per-head values. No GPU performance claim was added.

Post-#298 aliased score-V dispatch gradient contract (`34c6d60`): extend CPU-only Q/K-alias gradient parity through blocked, online, Triton-fallback, and CUDA-reference dispatch paths. No GPU performance claim was added.
...
Current evidence boundary through `d915d57`: #307–#323+ and the gen-bench threshold-sweep follow-ups add only CPU-safe prefetch, docs-v67/v68, sparse, AMP, blocked-prefill, packed shared-V online-decode, per-head score×V, sampler-layout, RoPE output-slot, compile, CUDA-build, duplicate-Q backward, forced-CPU measurement, packed decode dtype, AUTO-threshold, and harness-contract coverage. No CUDA run, GPU timing, GPU speedup, sparse-kernel result, or cold CUDA–Triton validation was added. Keep the real-GPU measurement / cold CUDA–Triton validation blocker at P0.

| P | Item | Why (from profile / notes) | Target | Risk |
|---|------|----------------------------|--------|------|
| **P0** | **Measure Triton/CUDA fused tril-score×V on real GPU** | Default **eager** still (**GPU blocker**; profile-v9, #100, #102, #104, #105, #110, #119, #120, #121, #123, #126, #127, #128, #129, #130, #131, #132, #133, #134, #135, and #136–#140 are CPU/docs/CI evidence only, and #141–#142 add CPU-only parity/configuration evidence, #143 is docs-only, #144 profile-v14 is flat CPU evidence, #145 adds only scriptable sparse-probe exit outcomes, #146 adds CPU-safe skip clarification, and #147 adds CPU-only online-decode parity, #148 is docs-only, #149 adds CPU-only AUTO sweep smoke, #150 adds CPU-safe Triton skip diagnostics, #151 is docs-only, and #152 confirms only the CPU generate copy ceiling; #153 is docs-only; #154 adds only CPU T>1 RoPE narrow reuse/parity coverage; #155 is docs-only; #156 adds only a CPU layout-materialization probe; #157 adds only CPU profile-v15 evidence; #158 is docs-only; #159 adds only CPU logger-fallback coverage; #160 adds only CPU packed-cache footprint accounting; #161 adds only CPU profile-v16 evidence; and #163 adds only structured GPU-measurement/skip scaffolding, #164 adds only packed-view decode preservation/parity coverage, and #165 adds only B>1 shared-V decode parity/accumulation coverage, #166 adds only CPU compile-probe fallback diagnostics, #167 is docs-only, #168 adds only CPU Triton skip-reason/strided-RoPE parity coverage, and #169 adds only CPU prefetch no-op/lifetime documentation, #170 adds only CPU packed-key-view online-decode parity; #171 and #172 are docs-only; #173 adds only CPU analytic-attention parity-matrix coverage; #174 adds only explicit CPU-extension setup smoke, with no CUDA timing; #175 and #177 are docs-only; #176 adds only CPU wide-head partial-tile parity; #178 adds only CPU-safe skip diagnostics and long-T fallback parity; and #179 adds only CPU AMP dtype/scaler contracts; #180 is docs-only; and #181 adds only CPU packed-footprint accounting; #182 adds only matched CPU profile-v17 evidence; #193 is docs-only; #194 adds only B=1 shared-V CPU epilogue coverage; #195 adds only CPU compile-probe diagnostics; #196 adds only CPU missing-nvcc setup-skip coverage; #197 is docs-only; #198 adds only CPU T=1 strict-tril backward contract coverage; #199 adds only CPU packed per-head decode GEMM parity; #200 is docs-only; #201 adds only CPU smoke timing-scope diagnostics; #202 is docs-only; #203 adds only CPU-safe unavailable-CUDA H2D gating; #204 adds matched CPU profile-v18 evidence; #205 adds CPU-only logger lifecycle contracts; #206 is docs-only; #207 adds CPU-only AMP failure-state coverage; #208 adds CPU-only blocked forward/gradient parity; #209 adds CPU-only RoPE skip diagnostics; #210 is docs-only; #211 adds CPU-only partial-page accounting; #212 adds CPU-only cold-shape validation; #213 adds CPU-only Triton skip-gate coverage; and #214 adds CPU-only sparse guardrail terminal coverage; and #215 adds CPU-only generate-benchmark environment-contract coverage; #216 adds CPU-only AUTO threshold lower-bound coverage; #217 is docs-only; #218 adds CPU-only raw-score decode contract coverage; #219 and #220 add CPU-only B=1 score×V view coverage; #221 adds CPU-only multi-step sampler-layout coverage; #224 and #225 add CPU-only boundary/fallback coverage; #228 adds matched profile-v19 counts; #229 adds strict-tril backward support coverage; and #230 adds packed per-head decode autograd parity coverage; #231 is docs-only; #232 adds structured backend skip results; and #233 adds unavailable-CUDA prefetch skip diagnostics; #234 adds CPU AMP skip-reason coverage; #235 adds long blocked/online gradient parity coverage; #236 is docs-only; #237 adds CPU-only sparse density-only probe coverage; #238 adds CPU-only AUTO environment cleanup coverage; #239 adds CPU-only eager raw-score decode coverage; #240 adds CPU-only explicit-backend AUTO-gate coverage): #270–#275 add only CPU-safe prefetch, GPU-skip, sparse-guardrail, blocked-tile, invalid gen-bench, and invalid-AMP-state contract coverage; no GPU timing or speedup evidence was added. attention `bmm` 30.50% / `mul` 30.78% / `copy_` 19.45%; forward `bmm` 34.12% / `mm` 24.93% / `mul` 17.69% / `copy_` 9.09%. #96–#123 add no GPU timing or speedup claim; #126–#142 add no GPU timing or speedup claim; #95 isolated forward `copy_`=24→18, while profile-v9 warmed harness is 16/call and generate is 558/call before #102. #100 lowers blocked/online inference epilogue buffering and tightens T=1 decode tiling; #102/#104 cut and re-measure generate `copy_` ~558→~398 with `cat` at 0; #105 confirms the residual-LN path without GPU evidence; #108 keeps wide-head cold Triton tiles bounded and removes shared-V CPU staging without GPU evidence; #110 cuts generate `copy_` ~398→~394 but leaves V-slot (~132/gen) and multinomial (~128/gen) copies at the probe ceiling; #114 adds only bounded CUDA cold-tile/build-smoke coverage; #116 adds only CPU shared-V T=1 epilogue/parity coverage; #117 adds only CPU compile-probe coverage; #119 adds only CPU AMP gating/bench coverage; #120 adds only CPU AUTO parity/harness coverage; #121 adds only CPU profile evidence; #123 adds only gated CPU sparse density evidence; #124 adds only CPU parity and skip-gated zero-stride Triton RoPE scaffolding; #125 and #126 are docs-only; #127 is CPU CI/profile plumbing; #128 is CPU parity/stride coverage; #129 is CPU profile evidence; #130 is a CPU skip/parity contract; #131 is a CPU AUTO harness extension; #132 and #133 are docs-only; #134 is no-`nvcc` build/config smoke; #135 is CPU B=1 score×V parity coverage; #136 is CPU compile/probe fallback diagnostics; #144 adds no GPU evidence, #145 keeps sparse default-off, and #146/#147 add no GPU evidence; #148 and #151 are docs-only, #149 adds only CPU AUTO sweep smoke, #150 adds CPU-safe Triton skip diagnostics, and #152 confirms only the CPU generate copy ceiling. GPU validation remains open. #93, #108, #110, #112, #116, #119, #121, #123, #127, #128, #129, #130, #131, #134, #135, and #136, #166, #168, #169, and #170 remain CPU-only evidence; #111, #113, #115, #118, #122, #125, #126, #132, #133, #137, and #167 are docs-only; #124 and #168 remain skip-gated GPU-readiness evidence; #169 remains CPU-only prefetch evidence; #173 remains CPU-only analytic-attention parity evidence; #174 remains CPU-only extension setup evidence; #176 remains CPU-only partial-tile parity; #178 remains CPU-only skip/fallback parity; #179 remains CPU-only AMP contract coverage; #180 is docs-only; #181 remains CPU-only packed-footprint evidence; #182 remains CPU-only profile-v17 evidence; #183 is docs-only; #184 remains CPU-only sparse-probe guardrail evidence; #185 remains CPU-only AUTO threshold-gate smoke; #186 remains CPU-only generate-benchmark validation; #187 is docs-only; #188 adds only CPU sampler-layout probe coverage; #189 adds only CPU packed shared-V decode parity; #190 is docs-only; #191 adds only CPU RoPE cache correctness/parity coverage; #193 is docs-only; #194 adds only CPU B=1 shared-V epilogue coverage; #195 adds only CPU compile-probe diagnostics; #196 adds only CPU missing-nvcc setup-skip coverage; #228 remains CPU-only profile-v19 evidence; #229 remains CPU-only strict-tril backward support coverage; and #230 remains CPU-only packed per-head decode autograd coverage; #231 is docs-only; #232 remains CPU-only structured backend skip coverage; and #233 remains CPU-only unavailable-CUDA prefetch coverage; #234 remains CPU-only AMP skip-reason coverage; #235 remains CPU-only blocked/online gradient parity coverage. | A100/H100: `bench_gpu_attn.py` (+ fused score×V) | Env blocker
| **P0** | **Cold CUDA/Triton tile/staging validation** | **Landed `opt/triton-cold` + `opt/triton-cold-v2` + #93 CPU tile flattening + #108 `opt/triton-cold-v3` + #114 `opt/cuda-cold-v3` + #120 strict AUTO gates:** adaptive power-of-2 tiles (grow @T≥256 pair #75/#79), wide-head 64×64 bounds, fused strict-tril score×V, shared-V view fallback without `(B*H,T,D)` staging, CPU→blocked adaptive, #114-aligned CUDA cold bounds, and #128 packed-stride T=1 decode coverage + #141 wide-head CPU parity + #170 packed-key-view coverage + #174 explicit CPU extension smoke + #176 blocked-tile-v4 partial-tile parity + #178 triton-cold-v5 skip/fallback parity; #177 is docs-only; #179 is CPU-only AMP contract coverage; #180 is docs-only; #181 is CPU-only packed-footprint accounting; #182 is CPU-only profile-v17 evidence; #193 is docs-only; #194 is CPU-only B=1 shared-V epilogue coverage; #195 is CPU-only compile-probe diagnostics; #196 is CPU-only missing-nvcc setup-skip coverage; #197 is docs-only; #198 is CPU-only T=1 strict-tril backward contract coverage; #199 is CPU-only packed per-head decode GEMM parity; #200 is docs-only; #201 is CPU-only smoke timing-scope coverage; #202 is docs-only; #203 is CPU-only H2D skip-gate coverage; #204 is CPU-only profile-v18 evidence; #205 is CPU-only logger lifecycle coverage; #206 is docs-only; #207 is CPU-only AMP failure-state coverage; #208 is CPU-only blocked forward/gradient parity; #209 is CPU-only RoPE skip-diagnostic coverage; #210 is docs-only; #211 is CPU-only partial-page accounting; #212 is CPU-only cold-shape validation; #213 is CPU-only Triton skip-gate coverage; #214 is CPU-only sparse guardrail terminal coverage; #215 is CPU-only generate-benchmark environment-contract coverage; #216 is CPU-only AUTO threshold lower-bound coverage; #217 is docs-only; #218 is CPU-only raw-score decode contract coverage; #219 and #220 are CPU-only B=1 score×V view parity/shape coverage; #221 is CPU-only multi-step sampler-layout coverage; #224 is CPU-only paired RoPE table-boundary coverage; #225 is CPU-only failed-backward compile-probe fallback coverage; #228 is CPU-only profile-v19 evidence; #229 is CPU-only strict-tril backward support coverage; #230 is CPU-only packed per-head decode autograd coverage; #231 is docs-only; #232 and #233 add CPU-only skip diagnostics; #234 adds CPU-only AMP skip-reason coverage; #235 adds CPU-only blocked/online gradient parity; #236 is docs-only; #237 adds CPU-only sparse density-only probe coverage; #238 adds CPU-only AUTO environment cleanup coverage; #239 adds CPU-only eager raw-score decode coverage; #240 adds CPU-only explicit-backend AUTO-gate coverage; #292 and #293 add CPU-only sparse-threshold and batched partial-tile contracts; #93 CPU blocked bench is 1.25× @256 / 4.00× @512 / 5.87× @1024, but GPU measurement and cold CUDA/Triton validation remain open. | A100/H100 microbench; bit-identical | Env blocker |
| **P1** | **`torch.compile` GPU train-step next** | **Landed #117 + #136 + #166 + #195**: `BDH_COMPILE_PROBE=train_bwd` probes the compiled forward/backward region; #136 makes construction, missing-target, and first-probe soft fallbacks identify the original eager module and documents the FULLGRAPH/AUTOGRAD boundary; #166 adds explicit `example_y`/forward-probe retry guidance; #195 sharpens missing-target and first-probe diagnostics and adds CPU eval-probe training-mode restoration coverage. Optimizer step and grad clearing remain eager. CPU guidance remains `COMPILE=1` **only with eager** + `MODE=default`; blocked/`reduce-overhead` warn. **GPU inductor / CUDA graphs still unmeasured.** | A100/H100: `BDH_COMPILE=0` vs `1` + `MODE=default` vs `reduce-overhead` + `FULLGRAPH` via `benchmarks/bench_train_step.py` | Low
| **P1** | **Decode GEMM / copy tax on generate** | **Host tax cut** #44+#48; **decode-gemm-v2** and #164 preserve packed KR/V strides in T=1 Triton decode without per-step contiguous staging, while #165 deepens B>1 shared-V accumulation in place and #170 preserves packed key/value views for non-flattenable online decode; #131 adds the CPU AUTO threshold sweep, #149 adds CPU-safe dispatcher smoke, #185 adds CPU-only per-child cold-gate fallback smoke, #186 adds CPU-only fail-closed generate checks, #187 is docs-only, #274 adds CPU-only invalid generate-selection validation, #188 adds CPU-only sampler-layout probe coverage, #189 adds CPU-only packed shared-V decode parity, #194 adds CPU-only B=1 shared-V epilogue coverage, and #152 confirms the remaining CPU-safe copy ceiling, #153 is docs-only, #154 reuses the warmed T>1 RoPE narrow without GPU evidence, #155 is docs-only, #156 only probes remaining CPU layout materializations, and #157 only records CPU profile-v15 evidence; #135 deepens the B=1 T=1 shared-V epilogue alongside **decode-mm** + **decode-online-v2** + **attn-auto** + **triton-decode-v3** + **cuda-decode-v3** + **`opt/prefill-blocked`** + **`opt/auto-tune`**: AUTO long-T cold+decode (adaptive blocked cold @T≥256), with optional independent cold threshold. CPU e2e AUTO **1.26×@1024 / 1.39×@2048**; IMPL=blocked ~1.23–1.43×; short A/B ~1.25× @1024. Remaining = **GPU** measure / re-tune thr. Default still eager. | A100/H100: `bench_generate.py --mode auto-ab` + `--mode impls` + `bench_gpu_attn.py --mode decode`; keep cat-free | Medium |
| **P1** | **CUDA prefetch H2D overlap** | **Landed #92 + #139 + #203 + #233 + #270** + #291 `BDH_PREFETCH_H2D=1`: one pinned batch staged on a side stream with event handoff; #139 explicitly verifies that CPU is an identity/no-stream/event/device-lookahead no-op, and #203 gates unavailable CUDA before stream/event construction with actionable skip reasons; #233 also catches availability-probe `AssertionError`/`RuntimeError` before any stream/event allocation; #270 adds explicit `BDH_PREFETCH_H2D=0` no-probe coverage. No CPU throughput evidence or GPU overlap result exists; #291 only deepens the CPU-safe global opt-out-before-probing contract. | CUDA host: measure H2D overlap, lifetime safety, and end-to-end train throughput; keep `BDH_PREFETCH_H2D=1` opt-in to measurement only | Env blocker |
| **P2** | **Fused RoPE kernel** | Attn: `mul`/`copy_` from strided rotate. **Landed #90**, **#124**, **#138**, **#168**, and **#191**: no expand/stack, table-pair T=1, zero-stride cis-row Triton scaffold, conservative complete-tensor/device/dtype skip gates, actionable skip reasons, strided T=1 output parity, and blocked/PyTorch CPU fallback; default remains eager; #191 adds CPU-only paired/flat cache-alignment coverage. GPU fused-RoPE validation is still open. | GPU Triton microbench still open | Low–medium |
| **P2** | **Sparsity follow-through** | **Landed #145 sparse-v3 + #237 + #272:** stable success/guardrail-failure exit outcomes and coverage/notes build on the gated density probe from `opt/sparse-v2`; `BDH_SPARSE_PROBE` is explicit opt-in, short-train x=26.63% / xy=11.37% at step 150, and the conservative x≥20% / xy≥8% guardrail passes; #292 covers the inclusive threshold boundary without starting crossover work. CPU sparse **never reliably beat dense** → dense/default path and sparse **stay OFF**; GPU sparse remains unmeasured; #237 only verifies the CPU density-only completion boundary. | GPU sparse bench only if density ≪10% and a kernel win is demonstrated | Speculative |
...
| **P3** | **Hardware / dtype** | **Landed `opt/bf16-train`+#54 `opt/amp-deepen`+#142 `opt/amp-train-v3`+#179 `opt/amp-train-v4`+#207:** opt-in `BDH_AMP_DTYPE` with explicit float32/bf16/fp16 configuration coverage, CPU-safe backend skips, and GradScaler gated to CUDA fp16; fp32 defaults remain unchanged. GPU train throughput still open; #275 adds CPU-only invalid-dtype state-preservation coverage and #289 adds CPU-safe forward-only environment coverage. | GPU box microbench | Env |
...
| Docs refresh #190 | **Landed** `a618356` — docs-only refresh through #188; profile-v17 counts and the real-GPU P0 blocker preserved
| RoPE fuse v4 #191 (current tip) | **Landed** `39fe7f7` — CPU-only paired/flat T=1 cache-narrow alignment and T>1 invalidation coverage; eager defaults preserved and no GPU claim
| Docs refresh #193 | **Landed** `4bd410a` — docs-only refresh through #191; profile-v17 counts and the real-GPU P0 blocker preserved
| Score×V epilogue coverage #194 | **Landed** `4fda79f` — CPU-only B=1 shared-V packed-cache `out=` accumulation and grad-safe fallback coverage; no GPU timing
| Compile probe v5 #195 | **Landed** `4ae8ab2` — explicit CPU-only missing-target/first-probe soft-fallback diagnostics and eval-probe mode restoration coverage; no GPU/CUDA-graph timing
| CUDA build v3 #196 | **Landed** `4e9d3e4` — CPU-safe missing-nvcc setup discovery across CUDA_HOME/CUDA_PATH/PATH with explicit setup smoke; no GPU build or timing claim
| Docs refresh #197 | **Landed** `ccadc3a` — docs-only refresh through #196; profile-v17 counts and the real-GPU P0 blocker preserved
| Attn-bwd-v4 #198 | **Landed** `9ad9015` — CPU-only T=1 strict-tril backward contract coverage across eager, blocked, online, Triton-fallback, and CUDA-reference dispatch for shared-V and full-head-V layouts; no GPU timing or speedup claim
| Packed per-head decode GEMM parity #199 | **Landed** `opt/decode-gemm-v4` at `269eb2d` — CPU-only T=1 packed capacity-strided per-head K/V parity and stride/view contract coverage; no GPU timing or performance claim
| Docs refresh #200 | **Landed** `e253bba` — docs-only matrix refresh through #198; profile-v17 counts and the real-GPU P0 blocker preserved
| CPU smoke timing scope #201 | **Landed** `opt/gpu-measure-v3` at `c25aa9f` — schema-v2 forced-CPU smoke summaries report actual device, timing scope, and reason; no GPU timing or speedup claim
| Docs refresh #202 | **Landed** `3d3d1fe` — docs-only matrix refresh through #201; real-GPU P0 blocker preserved
| Prefetch v4 #203 | **Landed** `opt/prefetch-v4` at `9669e06` — CPU-safe unavailable-CUDA H2D skip gate before stream/event construction; no GPU overlap or timing claim
| Profile v18 #204 | **Landed** `opt/profile-v18` at `5995a7c` — matched CPU counts flat versus v17 (`copy_`=2/12/394 per call; `cat=0`; `contiguous=0`); no GPU measurement
| Zerograd logger lifecycle #205 | **Landed** at `e8ad099` — CPU-only one-shot partial-window flush and update-after-close rejection; no GPU measurement
| Docs refresh #206 | **Landed** at `6e0f313` — docs-only matrix refresh through #204; profile-v18 counts and the real-GPU P0 blocker preserved
| CPU AMP failure-state contract #207 | **Landed** at `dd11718` — unavailable CPU requests preserve complete prior AMP state; CPU-only contract coverage with no GPU measurement
| Blocked-tile long-path gradient parity #208 | **Landed** at `dd4fc90` — CPU-only exact forward/gradient parity at the tiled boundary; no GPU measurement
| Triton RoPE skip-reason contract #209 | **Landed** at `a48b7e1` — CPU-safe primary-input diagnostic before runtime probing; no GPU measurement
| Docs refresh #210 | **Landed** at `fecfba4` — docs-only matrix refresh through #209; no code or GPU evidence
| Partial cache-page accounting #211 | **Landed** at `d3a37b7` — CPU-only non-page-aligned final-page grow/copy-byte contract; no GPU measurement
| CUDA cold shape contract #212 | **Landed** at `61af5e0` — CPU-only validation before optional native dispatch; no GPU measurement
| Triton cold skip-gate contract #213 | **Landed** at `bd809b9` — deterministic CPU-safe import/device branch coverage; no GPU measurement
| Sparse guardrail terminal contract #214 | **Landed** at `8ec4b16` — failed enforced density guardrails stop before CPU crossover work; no GPU measurement
| Generate-benchmark environment contracts #215 | **Landed** at `eafe65c` — interrupted/AUTO dispatch restores environment state; CPU-only, no GPU measurement
| AUTO threshold lower-bound coverage #216 | **Landed** at `265b25c` — negative decode/cold thresholds reject before dispatch; CPU-only, no GPU measurement
| Docs refresh #217 | **Landed** at `fb80c34` — docs-only matrix refresh through #215; profile-v18 counts and the real-GPU P0 blocker preserved
| Online decode raw-score contract #218 | **Landed** at `50d40f5` — blocked/online/Triton-fallback/CUDA-reference paths reject softmax/scale semantics; CPU-only, no GPU measurement
| B=1 non-flat score×V view coverage #219 | **Landed** at `d218150` — non-flat packed-K views preserve the 4-D path without staging; CPU-only parity, no GPU measurement
| B=1 score×V view alignment #220 | **Landed** at `f344494` — non-flat K views retain tiled `bmm` shape parity with eager decode; CPU-only, no GPU measurement
| Multi-step sampler layout contract #221 | **Landed** at `39ca536` — two-token CPU profile preserves one `(B,)` result per token, `aten::cat=0`, and no `(B,V)` sampler materialization; no GPU measurement
| Paired RoPE table-boundary guards #224 | **Landed** at `ad7a50b` — negative/out-of-range paired T=1 lookups are rejected without poisoning valid flat or paired cache narrows; no GPU measurement
| Failed-backward compile-probe fallback #225 | **Landed** at `258b24c` — partial parameter gradients are cleared before the eager fallback and caller training mode is preserved; CPU-only, no GPU compile measurement
| Profile-v19 #228 | **Landed** at `1ad6b01` — matched CPU attention/forward/generate `copy_` counts remain 2/12/394 per call with `cat=0` and `contiguous=0`; flat versus v18, no GPU measurement
| Strict-tril backward support #229 | **Landed** at `26d0b4f` — single-query loss reaches only the matching Q row and strict-past K/V rows across supported dispatches and V layouts; CPU-only, no GPU measurement
| Packed per-head decode GEMM parity #230 | **Landed** at `0a42a67` — capacity-strided per-head K/V decode outputs and gradients match eager at the tiled oneshot boundary across eager/blocked/online/Triton-fallback; CPU-only, no GPU measurement
| Docs refresh #231 | **Landed** at `d8eb80e` — docs-only matrix refresh through #230; profile-v19 counts and the real-GPU P0 blocker preserved
| Structured backend skip results #232 | **Landed** at `efbb1f3` — schema-v3 run/backend skip records, explicit result status/reason, and null timing for unavailable backends; CPU-only, no GPU measurement
| Prefetch v5 CUDA skip diagnostics #233 | **Landed** at `ff5b071` — availability-probe exceptions return allocation-free H2D skip outcomes before stream/event construction; CPU-only, no H2D overlap measurement
| CPU AMP skip-reason contract #234 | **Landed** at `1f1edd9` — unavailable bfloat16/float16 requests retain requested dtype and backend context; CPU-only, no GPU measurement
| Long blocked/online gradient parity #235 | **Landed** at `ee586d0` — T=257 shared/head-matched V cases cover blocked and online aliases; CPU-only, no GPU measurement
| Docs refresh #236 | **Landed** at `02bcf68` — docs-only matrix refresh through #235; profile-v19 counts and the real-GPU P0 blocker preserved
| Sparse density-only probe boundary #237 | **Landed** at `66c1680` — passing guardrail completes before CPU crossover work; sparse remains opt-in/default-off, no GPU measurement
| Generate AUTO environment cleanup #238 | **Landed** at `95ecae0` — interrupted AUTO runs restore initially-unset variables; CPU-only, no GPU measurement
| Eager raw-score decode contract #239 | **Landed** at `34a0d9f` — eager joins the no-softmax/no-scale decode parity contract; CPU-only, no GPU measurement
| AUTO cold explicit-backend contract #240 | **Landed** at `2a004c3` — explicit backends remain authoritative on both AUTO gates; CPU-only, no GPU measurement
| Top-k-one sampler layout #241 | **Landed** at `8a2ea37` — CPU sampler-layout coverage now includes the `top_k=1` boundary alongside scaled, narrow, and full-vocabulary cases; no GPU measurement
| Distinct-Q/K score-V parity #242 | **Landed** at `c20f440` — eager, blocked, and online score×V paths match the raw expected result for non-aliased query/key tensors; CPU-only, no GPU measurement
| Blocked RoPE tile contract #244 | **Landed** at `9d2fcb5` — nonpositive blocked tiles are rejected before the T=1 shortcut and covered at T=1 and T>1; CPU-only, no GPU measurement
| Successful compile-backward probe cleanup #245 | **Landed** at `4047ce9` — a successful `train_bwd` probe restores eval caller mode and clears parameter gradients; CPU-only, no GPU compile measurement
| CUDA missing-nvcc skip deepen | **Landed** at `d6a5455` — a directory named `nvcc` remains a clear CPU-safe no-op before CUDAExtension setup; no GPU build or timing claim
| Shared-V strict-tril backward #247 | **Landed** at `77a1ca4` — CPU-only shared-V strict-tril backward parity coverage; no GPU timing or speedup claim
| Docs refresh #248 | **Landed** at `0c7c2f1` — docs-only matrix refresh through the prior tip
| CUDA-mirror packed decode gradients #249 | **Landed** at `567c1bf` — CPU CUDA-mirror forward and Q/K/V autograd parity for packed per-head T=1 decode; no CUDA timing or performance claim
| Profile-v20 | **Landed** at `f4c7cab` — matched CPU attention/forward/generate `copy_` counts remain 2/12/394 per call with `cat=0` and `contiguous=0`, flat versus profile-v19; no GPU timing or speedup claim
| Force-CPU GPU harness #251 | **Landed** at `84bd3f6` — forced smoke remains on CPU even when CUDA is available; schema-v3 contract coverage, no GPU timing claim
| CPU prefetch gate #252 | **Landed** at `0baa35d` — CPU H2D checks return before probing CUDA availability; no H2D timing or overlap claim
| Docs refresh #253 | **Landed** at `53491cf` — docs-only matrix refresh through profile-v20, #251, and #252
| Active AMP state on CPU failure #254 | **Landed** at `94a4e55` — unavailable dtype switches preserve the complete active AMP configuration; CPU-only, no GPU throughput claim
| Absent generate-impl cleanup #255 | **Landed** at `1b67d13` — interrupted implementation sweeps restore initially absent environment state; CPU-only, no GPU generate claim
| Batched blocked-tile gradient parity #256 | **Landed** at `b00dbd1` — long blocked/online CPU parity extends to `B=2` wide-head layouts; no GPU timing claim
| Sparse density-only crossover boundary #257 | **Landed** at `042a01d` — CPU-only boundary coverage; sparse remains opt-in/default-off and no GPU claim
| AUTO threshold fallback #258 | **Landed** at `6abd53b` — unset cold thresholds track later decode-threshold changes; CPU-only contract coverage, no GPU timing claim
| Score-V gradients for distinct Q/K #259 | **Landed** at `f5a9cea` — blocked/online CPU paths preserve Q/K/V gradients; no GPU timing claim
| Signed raw-score decode #260 | **Landed** at `9b6d0a8` — CPU decode coverage keeps signed raw scores and independent per-head values across supported dispatches; no GPU timing claim
| Sampler-layout overflow #262 | **Landed** at `1384ca5` — CPU-only coverage extends the `top_k` greater-than-vocabulary clamp boundary; no GPU timing or performance claim
| Mixed-dtype RoPE output #263 | **Landed** at `7c0e969` — CPU-only fp16-to-fp32 cache-style output coverage across entrypoints and sequence shapes; no GPU timing claim
| Failed compile-probe eval cleanup #264 | **Landed** at `3747356` — failed `train_bwd` probes restore eval mode and clear partial gradients before eager fallback; no GPU compile claim
| CUDA_PATH missing-nvcc skip #265 | **Landed** at `6d6aaab` — stale non-executable `CUDA_PATH` `nvcc` remains a CPU-safe no-op before CUDAExtension setup; no GPU build or timing claim
| Strict-tril backward boundary queries #266 | **Landed** at `cbf0831` — CPU-only locality coverage spans first, interior, and final query rows across supported dispatches and V layouts; no GPU timing claim
| Tiled packed per-head decode gradients #268 | **Landed** at `aebc449` — CPU-only autograd parity covers capacity-strided T=1 decode GEMMs across blocked, online, and Triton-fallback paths; no GPU timing claim
| Explicit prefetch H2D opt-out #270 | **Landed** at `dae2e07` — CPU-safe no-probe coverage for `BDH_PREFETCH_H2D=0`; no H2D timing or overlap claim
| Run-level GPU skip status #271 | **Landed** at `75597c9` — unavailable-device summaries report schema-v4 `status: skip`; no GPU timing or speedup evidence
| Sparse guardrail terminal contract #272 | **Landed** at `b9bf6c8` — enforced density failures stop before CPU crossover work; sparse remains opt-in/default-off
| Blocked-tile raw-score contract #273 | **Landed** at `3b3c082` — deterministic CPU tile-boundary raw strict-tril score×V coverage; no GPU timing claim
| Invalid generate selection #274 | **Landed** at `81e0005` — fail-closed CPU validation before model/device setup; no GPU performance claim
| Invalid AMP state #275 | **Landed** at `915fe71` — CPU-only invalid-dtype failure-state preservation; no GPU throughput claim
| Docs refresh #277 | **Landed** at `aeec930` — matrix/backlog docs through #275; docs-only
| AUTO threshold override #278 | **Landed** at `abef868` — CPU-safe explicit-cold independence/fallback coverage; no GPU timing claim
| Aliased score-V gradients #279 | **Landed** at `8d4ea64` — CPU blocked/online Q/V gradient parity when Q and K alias; no GPU timing claim
| Multi-query online decode #280 | **Landed** at `4de8f5a` — CPU-only signed raw-score/per-head past-value contract; no GPU timing claim
| Strided sampler output #281 | **Landed** at `76e02e6` — CPU-only default/top-k non-contiguous decode-narrow contract; no GPU timing claim
...
| Shared-V tiled decode gradients #288 | **Landed** `945ba03` — CPU-safe B>1 autograd parity for capacity-strided shared-V T=1 decode; no GPU timing or performance claim |
| AMP forward-only environment contract #289 | **Landed** `460304c` — CPU-safe environment/default/override coverage; no GPU throughput claim |
| GPU-harness device-local synchronization #290 | **Landed** `2f09e47` — CPU smoke contract only; no CUDA timing result on this box |
| Global prefetch H2D opt-out #291 | **Landed** `85f422a` — CPU-safe opt-out before CUDA probing or stream/event allocation; GPU overlap remains unmeasured |
| Sparse guardrail threshold boundary #292 | **Landed** `125c8b7` — CPU-only inclusive threshold coverage; sparse remains opt-in/default-off with no GPU claim |
| Batched blocked-tile contract #293 | **Landed** `014a724` — CPU-only broadcast/head-matched V raw strict-tril coverage at a non-divisible partial tile; no GPU performance claim |
| Gen-bench AUTO input contract #294 | **Landed** `8fedb1a` — CPU-safe invalid prompt/threshold validation before model/device setup; no GPU performance claim |
| AUTO blank cold-threshold contract #295 | **Landed** `cde4388` — CPU-safe empty/whitespace fallback coverage after live decode-threshold updates; no GPU performance claim
| Docs refresh #296 | **Landed** `d8cc8d1` — docs-only refresh through #294
...
| Current tip | **Landed** at `d915d57` — profile-v20 counts remain 2/12/394 with `cat=0` and `contiguous=0`; #307–#323+ and threshold-sweep follow-ups add CPU-safe contract coverage only; the real-GPU P0 blocker remains open |
...
| #319 compile option forwarding | **Landed** at `2439f2e` — CPU-only successful `train_bwd` probe forwards mode/fullgraph options and cleans up probe state; no GPU/CUDA-graph measurement
| #320 CUDA-build flag precedence | **Landed** at `89584c6` — `BDH_FORCE_CPU_EXT=1` takes precedence over `BDH_BUILD_CUDA=1`; CPU-only setup contract, no GPU build or timing claim
| #321 duplicate-Q backward | **Landed** at `3f5dbe8` — CPU-safe duplicate-Q strict-tril single-query gradient-routing contract; no GPU timing or performance claim
| #322 forced-CPU measurement boundary | **Landed** at `b6abb22` — forced-CPU smoke skips GPU metadata initialization when CUDA reports available and preserves CPU timing labels; no GPU timing or speedup claim
| #323 packed decode GEMM dtype | **Landed** at `d915d57` — CPU-only float16/bfloat16 parity across broadcast and per-head packed values; no GPU timing or performance claim
