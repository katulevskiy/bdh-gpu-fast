...
| **P3** | **Hardware / dtype** | **Landed `opt/bf16-train`+#54 `opt/amp-deepen`+#142 `opt/amp-train-v3`+#179 `opt/amp-train-v4`+#207 + #326:** opt-in `BDH_AMP_DTYPE` with explicit float32/bf16/fp16 configuration coverage, CPU-safe backend skips, and GradScaler gated to CUDA fp16; fp32 defaults remain unchanged. GPU train throughput still open; #275 adds CPU-only invalid-dtype state-preservation coverage and #289 adds CPU-safe forward-only environment coverage. | GPU box microbench | Env |
...
| #324 docs-v69 | **Landed** at `2b39cbd` — docs-only refresh through #323; no GPU evidence
| #325 prefetch identity | **Landed** at `6f0f57d` — CPU DataLoader H2D identity/no-probe coverage; no H2D timing or overlap claim
| #326 AMP forward-only train step | **Landed** at `727f36f` — CPU-safe logits-only autocast, fp32 CE, and post-step gradient-clearing coverage; no GPU throughput claim
| #327 blocked-prefill V layouts | **Landed** at `6569a51` — CPU raw strict-tril parity for shared and head-matched V layouts; no GPU performance claim
| #328 sparse falsey gate | **Landed** at `0190637` — false-like `BDH_SPARSE_PROBE` values remain default-off no-ops; no sparse or GPU claim
