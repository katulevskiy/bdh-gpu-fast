...
def test_batched_strict_tril_raw_score_backward_contract_partial_tile(
    impl, value_heads
):
    """Batched partial tiles keep strict-tril gradients source-aligned."""
    B, H, T, N, D = 2, 2, 7, 2, 2
    Q = torch.arange(1, 1 + B * H * T * N, dtype=torch.float64).view(
        B, H, T, N
    ).requires_grad_()
    K = torch.arange(101, 101 + B * H * T * N, dtype=torch.float64).view(
        B, H, T, N
    ).requires_grad_()
    V = torch.arange(201, 201 + B * value_heads * T * D, dtype=torch.float64).view(
        B, value_heads, T, D
    ).requires_grad_()
...
@pytest.mark.parametrize("impl", ["blocked", "online"])
@pytest.mark.parametrize("value_heads", [1, 2])
def test_bdh_attn_blocked_autograd_self_alias_prefill_contract(impl, value_heads):
    """Long prefill keeps analytic backward parity when Q and K are aliased."""
    B, H, T, N, D = 2, 2, 257, 3, 4
    g = torch.Generator().manual_seed(307 + value_heads)
    ...
    got, got_grads = run(
        lambda Q, V: bdh_attn(Q, Q, V, impl=impl, use_autograd_fn=True)
    )
...
@pytest.mark.parametrize("impl", ["blocked", "online"])
@pytest.mark.parametrize("value_heads", [1, 2])
def test_bdh_attn_blocked_autograd_preserves_padded_v_storage(impl, value_heads):
    """Dispatcher-selected long prefill preserves padded V-view gradients."""
    B, H, T, N, D = 2, 2, 257, 3, 4
    g = torch.Generator().manual_seed(317 + value_heads)
    Q0 = torch.randn(B, H, T, N, dtype=torch.float64, generator=g)
    K0 = torch.randn(B, H, T, N, dtype=torch.float64, generator=g)
...
@pytest.mark.parametrize("impl", ["blocked", "online"])
@pytest.mark.parametrize("value_heads", [1, 2])
def test_bdh_attn_blocked_autograd_preserves_padded_qk_storage(impl, value_heads):
    """Dispatcher-selected long prefill preserves padded Q/K-view gradients."""
    T, B, H, N, D = 257, 2, 2, 5, 4
    g = torch.Generator().manual_seed(327 + value_heads)
    Q_storage0 = torch.randn(
...
def test_cpu_long_padded_qk_autograd_matches_eager(impl, value_heads):
    ...
    T, B, H, N, D = 257, 2, 2, 5, 4
    g = torch.Generator().manual_seed(129 + value_heads)
    Q_storage0 = torch.randn(
        B, H, T, N + 1, dtype=torch.float64, generator=g
    )
    K_storage0 = torch.randn(
...
def test_auto_cold_dispatch_preserves_padded_prefill_autograd(
    ...
    """AUTO-selected cold tiles preserve gradients through capacity-padded views."""
    monkeypatch.setenv("BDH_ATTN_AUTO", "1")
    monkeypatch.setenv("BDH_ATTN_AUTO_COLD_THRESHOLD", "256")
    monkeypatch.delenv("BDH_ATTN_IMPL", raising=False)
    _bump()
    ...
    T, B, H, N, D = 257, 2, 2, 3, 4
    g = torch.Generator().manual_seed(357 + value_heads)
    Q_storage0 = torch.randn(B, H, T + 1, N, dtype=torch.float64, generator=g)
    K_storage0 = torch.randn(B, H, T + 1, N, dtype=torch.float64, generator=g)
    V_storage0 = torch.randn(
        B, value_heads, T + 1, D, dtype=torch.float64, generator=g
    )
    weight = torch.randn(B, H, T, D, dtype=torch.float64, generator=g)
