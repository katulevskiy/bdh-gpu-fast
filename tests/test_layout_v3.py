...
def test_sampler_padded_nonunit_probability_buffer_preserves_zero_stride_output():
    """Padded scratch and singleton-stride decode output may be combined safely."""
    torch.manual_seed(0)
    logits_storage = torch.randn(2, 32, 2)
    logits = logits_storage[..., 0]
    probs_storage = torch.full((2, 34, 2), -777.0)
...
def test_sampler_combines_padded_nonunit_logits_probs_and_output():
    """Sampler must isolate three independent strided layout views."""
    torch.manual_seed(0)
    logits_storage = torch.randn(2, 34, 2)
    logits = logits_storage[:, 1:-1, 0]
    probs_storage = torch.full((2, 34, 2), -777.0)
    probs_buf = probs_storage[:, 1:-1, 0]
    logits_before = logits_storage.clone()
    probs_before = probs_storage.clone()
    assert logits.stride() == (68, 2)
    assert logits.storage_offset() == 2
    assert probs_buf.stride() == (68, 2)
    ...
    for name, kwargs in cases:
        logits_storage.copy_(logits_before)
        probs_storage.copy_(probs_before)
        destination = torch.full((1, 3, 2), -123, dtype=torch.long)
        destination_before = destination.clone()
        idx_out = destination[:, 1, :].transpose(0, 1)
        ...
        assert torch.equal(logits_storage[:, 0], logits_before[:, 0]), name
        assert torch.equal(logits_storage[:, -1], logits_before[:, -1]), name
        assert torch.equal(logits_storage[..., 1], logits_before[..., 1]), name
        assert torch.equal(probs_storage[:, 0], probs_before[:, 0]), name
        assert torch.equal(probs_storage[:, -1], probs_before[:, -1]), name
        assert torch.equal(probs_storage[..., 1], probs_before[..., 1]), name
        ...
        torch.manual_seed(17)
        ref = bdh.BDH._sample_from_logits(
            logits_before[:, 1:-1, 0].contiguous(),
            **kwargs,
            probs_buf=torch.empty(2, 32),
            softmax=torch.nn.functional.softmax,
            multinomial=torch.multinomial,
